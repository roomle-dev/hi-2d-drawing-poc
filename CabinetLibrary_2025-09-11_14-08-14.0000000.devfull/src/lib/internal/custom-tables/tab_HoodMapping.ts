...
export interface cti_tab_HoodMapping {
  readonly in_Supplier? : string;
  readonly in_HoodId? : string;
}

export interface ctm_tab_HoodMapping {
}

export interface cto_tab_HoodMapping extends ctm_tab_HoodMapping {
  readonly _id : number;
  readonly GraphicId? : string;
}

export interface ICT_tab_HoodMapping
extends cti_tab_HoodMapping, cto_tab_HoodMapping {}

export class ct2_tab_HoodMapping {

public findExactly(
     in_Supplier: string|undefined,
     in_HoodId: string|undefined,
): cto_tab_HoodMapping | undefined {
  const res = ct_tab_HoodMapping.find((p) => 
...
public find(
predicate: (value: cti_tab_HoodMapping) => boolean
): cto_tab_HoodMapping | undefined {
  for (let index = 0; index < ct_tab_HoodMapping.length; index++){
    const element = ct_tab_HoodMapping[index];
...
export var ct_tab_HoodMapping :ICT_tab_HoodMapping[] = [
{
 _id: 1,
 in_Supplier: "Siemens",
 in_HoodId: "LC96BBM50",

...
export interface IP_part_Handle_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Handle01;
  get pa_HardwareId(): string;
  get pa_TopColor(): string;
}

export interface IP_part_Handle_PartVarsWritable extends IP_part_Handle_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_TopColor(value: string);
}

export class P_part_Handle extends PartBase implements IP_part_Handle_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Handle01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Handle', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_Handle (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Handle (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM152 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Handle_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Add the handle to BOM";
        _bom.set(partSelf);

...
import { IPartParentsNonNull_mc_Fliplift01 } from '../mod-interfaces'

export interface IP_part_FlipliftUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Fliplift01;
  get pa_BomId(): string;
  get pa_PartgroupBomId(): string;
}

export interface IP_part_FlipliftUnit_PartVarsWritable extends IP_part_FlipliftUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
  set pa_PartgroupBomId(value: string);
}

export class P_part_FlipliftUnit extends PartBase implements IP_part_FlipliftUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Fliplift01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FlipliftUnit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Fliplift01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FlipliftUnit (id: ' + this._id + ')');
    }
    if (this.#pa_PartgroupBomId === undefined) {
      logError('Attribute pa_PartgroupBomId not set in part part_FlipliftUnit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 206
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM206 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FlipliftUnit_PartVarsReadOnly) {
            const part = partSelf;
...
this.bom_ElementCategory=elementCategory
        ...
        _bom._description = "Partgroup of fliplift";
        _bom.set(partSelf);

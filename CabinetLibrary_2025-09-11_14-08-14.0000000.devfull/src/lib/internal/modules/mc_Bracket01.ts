...
import { IP_part_ShelvesBrackets_PartVarsWritable, P_part_ShelvesBrackets } from '../parts/part_ShelvesBrackets'
import { IModuleNonNull_mc_Bracket01,IModParents_mc_Bracket01,IPartParentsNonNull_mc_Bracket01} from '../mod-interfaces'
import {IModVar_mod_Width,IModVarNonNull_mod_Width,IModVar_mod_Depth,IModVarNonNull_mod_Depth,IModVar_mod_Height,IModVarNonNull_mod_Height,IModVar_mod_ShelvesBrackets,IModVarNonNull_mod_ShelvesBrackets,IModVar_mod_ShelvesThk,IModVarNonNull_mod_ShelvesThk,IModVar_mod_ShelvesColor,IModVarNonNull_mod_ShelvesColor} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_Width, IMatrix_mod_Depth, IMatrix_mod_Height, IMatrix_mod_ShelvesBrackets, IMatrix_mod_ShelvesThk, IMatrix_mod_ShelvesColor} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_ShelvesBrackets } from '../part-add-interfaces';
import { mc_Bracket01_createBuildPlan, mc_Bracket01_afterDataCompletion, mc_Bracket01_manufacturerDataCompletion } from '../../modules/mc_Bracket01';

export interface cbp_mc_Bracket01 extends IModBaseProp
, IPartAdd_part_ShelvesBrackets, IModVarNonNull_mod_Width, IModVarNonNull_mod_Depth, IModVarNonNull_mod_Height, IModVarNonNull_mod_ShelvesBrackets, IModVarNonNull_mod_ShelvesThk, IModVarNonNull_mod_ShelvesColor {
...
export interface dc_mc_Bracket01 extends IModBaseProp
, IModVar_mod_Width, IModVar_mod_Depth, IModVar_mod_Height, IModVar_mod_ShelvesBrackets, IModVar_mod_ShelvesThk, IModVar_mod_ShelvesColor {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mc_Bracket01;
  parent: IModParents_mr_Shelves;
}

export interface adc_mc_Bracket01 extends IModBaseProp
...
export class OD_M_mc_Bracket01 extends OD_Base  implements dc_mc_Bracket01
,IModParents_mc_Bracket01
,IModVar_mod_Width,IModVar_mod_Depth,IModVar_mod_Height,IModVar_mod_ShelvesBrackets,IModVar_mod_ShelvesThk,IModVar_mod_ShelvesColor
 {
  constructor(parent: IModParents_mr_Shelves, manufacturerMode?:boolean) {
    super('mc_Bracket01', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_Bracket01_NonNull(this);
  }
  parent: IModParents_mr_Shelves;
  _selfNonNull: OD_M_mc_Bracket01_NonNull;

  override variants:string[] = ['mod_Width','mod_Depth','mod_Height','mod_ShelvesBrackets','mod_ShelvesThk','mod_ShelvesColor',];
  #mod_Width?: number;
  ...
  #manufacturerDataCompletionInternal = mc_Bracket01_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_Bracket01_NonNull(this);
  ...
  seal(): IModuleNonNull_mc_Bracket01{
    ...
    const adc = new OD_M_mc_Bracket01_NonNull(this);
    return adc;
  }
  ...
  addpart_ShelvesBrackets(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_ShelvesBrackets_PartVarsWritable {
    var c = new P_part_ShelvesBrackets(this._selfNonNull, x, y, z, dimx, dimy, dimz);
    this.p.push(c);
    return c;
  }

  override createBuildPlan():void {
    var cbp = new OD_M_mc_Bracket01_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_Bracket01_NonNull implements cbp_mc_Bracket01, adc_mc_Bracket01 ,IPartParentsNonNull_mc_Bracket01 {
  constructor(parent: OD_M_mc_Bracket01) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_Bracket01;
  ...
  #createBuildPlanInternal = mc_Bracket01_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_Bracket01_afterDataCompletion;
  ...
  parent: IModParents_mr_Shelves;
  addpart_ShelvesBrackets(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_ShelvesBrackets_PartVarsWritable {
    return this.#internalParent.addpart_ShelvesBrackets(x, y, z, dimx, dimy, dimz);
  }

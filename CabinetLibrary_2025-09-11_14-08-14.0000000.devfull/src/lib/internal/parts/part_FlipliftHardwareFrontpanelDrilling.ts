...
export interface IP_part_FlipliftHardwareFrontpanelDrilling_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_FlipliftHardware01;
  get pa_ProcessingId(): string;
}

export interface IP_part_FlipliftHardwareFrontpanelDrilling_PartVarsWritable extends IP_part_FlipliftHardwareFrontpanelDrilling_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
}

export class P_part_FlipliftHardwareFrontpanelDrilling extends PartBase implements IP_part_FlipliftHardwareFrontpanelDrilling_PartVarsWritable {
...
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FlipliftHardwareFrontpanelDrilling', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FlipliftHardwareFrontpanelDrilling_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_FlipliftHardwareFrontpanelDrilling (id: ' + this._id + ')');
    }

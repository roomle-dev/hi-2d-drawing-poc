...
export interface IP_part_DrawerShelfbtmConnectorDrilling_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_DrawerBox01;
  get pa_ProcessingId(): string;
}

export interface IP_part_DrawerShelfbtmConnectorDrilling_PartVarsWritable extends IP_part_DrawerShelfbtmConnectorDrilling_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
}

export class P_part_DrawerShelfbtmConnectorDrilling extends PartBase implements IP_part_DrawerShelfbtmConnectorDrilling_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerShelfbtmConnectorDrilling', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_DrawerShelfbtmConnectorDrilling_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_DrawerShelfbtmConnectorDrilling (id: ' + this._id + ')');
    }

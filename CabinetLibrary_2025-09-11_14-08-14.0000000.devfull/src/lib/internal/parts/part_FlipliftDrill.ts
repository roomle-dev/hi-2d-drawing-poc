...
export interface IP_part_FlipliftDrill_PartVarsReadOnly extends IPartBase {
  parent: IModBaseProp;
}

export interface IP_part_FlipliftDrill_PartVarsWritable extends IP_part_FlipliftDrill_PartVarsReadOnly {
}

export class P_part_FlipliftDrill extends PartBase implements IP_part_FlipliftDrill_PartVarsWritable {
  constructor(parent: IModBaseProp, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FlipliftDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }

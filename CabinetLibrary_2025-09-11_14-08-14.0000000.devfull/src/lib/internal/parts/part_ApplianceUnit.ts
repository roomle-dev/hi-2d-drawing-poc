...
import { IPartParentsNonNull_mc_Dishwasher } from '../mod-interfaces'

export interface IP_part_ApplianceUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Dishwasher;
  get pa_BomId(): string;
}

export interface IP_part_ApplianceUnit_PartVarsWritable extends IP_part_ApplianceUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_ApplianceUnit extends PartBase implements IP_part_ApplianceUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Dishwasher, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_ApplianceUnit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Dishwasher; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_ApplianceUnit_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_ApplianceUnit (id: ' + this._id + ')');
    }

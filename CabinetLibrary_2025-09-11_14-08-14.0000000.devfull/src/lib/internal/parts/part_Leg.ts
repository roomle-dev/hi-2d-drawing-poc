...
import { IPartParentsNonNull_mc_Leg01 } from '../mod-interfaces'

export interface IP_part_Leg_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Leg01;
  get pa_HardwareId(): string;
  get pa_TopColor(): string;
}

export interface IP_part_Leg_PartVarsWritable extends IP_part_Leg_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_TopColor(value: string);
}

export class P_part_Leg extends PartBase implements IP_part_Leg_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Leg01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Leg', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_Leg01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_Leg (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Leg (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM151 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Leg_PartVarsReadOnly) {
            const part = partSelf;
            internal_enterBomPartMasterDataElements(partSelf._id, partSelf._partId);
             // ####################### CUSTOM SCRIPTS ########################
// Filter function to get all the hardware elements for the leg
...
HardwareElements.forEach(info => {
  ...
  let LegBOMInfo = GlobalFunc.find_HardwareLibrary(info.SupplierArticleNumber!,info.Supplier!);
  let LegBOMName = GlobalFunc.find_PartSettings("part_Leg");

  elem.bom_Supplier=info.Supplier;
  elem.bom_SupplierArticle=info.SupplierArticleNumber;
  elem.bom_Description1=LegBOMInfo.Description;
  elem.bom_Description2=LegBOMInfo.Description2;
  elem.bom_Length=LegBOMInfo.Length;
  elem.bom_Width=LegBOMInfo.Width;
  elem.bom_Thk=LegBOMInfo.Thickness;
  elem.bom_Color=LegBOMInfo.Color;
  elem.bom_Weight=LegBOMInfo.Weight;
  elem.bom_Name=LegBOMName.BomPartDescription;
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Add all the elements for the leg to BOM";
        _bom.set(partSelf);

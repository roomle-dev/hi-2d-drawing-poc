...
export interface IP_part_FinishPanelProfile_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Upright01;
  get pa_BomId(): string;
}

export interface IP_part_FinishPanelProfile_PartVarsWritable extends IP_part_FinishPanelProfile_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_FinishPanelProfile extends PartBase implements IP_part_FinishPanelProfile_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Upright01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FinishPanelProfile', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FinishPanelProfile_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FinishPanelProfile (id: ' + this._id + ')');
    }

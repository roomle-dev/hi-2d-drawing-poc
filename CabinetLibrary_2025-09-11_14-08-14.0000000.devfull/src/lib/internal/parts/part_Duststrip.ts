...
import { IPartParentsNonNull_mc_Duststrip01 } from '../mod-interfaces'

export interface IP_part_Duststrip_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Duststrip01;
  get pa_HardwareId(): string;
}

export interface IP_part_Duststrip_PartVarsWritable extends IP_part_Duststrip_PartVarsReadOnly {
  set pa_HardwareId(value: string);
}

export class P_part_Duststrip extends PartBase implements IP_part_Duststrip_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Duststrip01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Duststrip', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_Duststrip01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_Duststrip (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM180 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Duststrip_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Add the duststrip to BOM";
        _bom.set(partSelf);

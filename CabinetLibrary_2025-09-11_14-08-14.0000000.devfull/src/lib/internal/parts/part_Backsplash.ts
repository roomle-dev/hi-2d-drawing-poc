...
import { IPartParentsNonNull_mc_Backsplash } from '../mod-interfaces'

export interface IP_part_Backsplash_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Backsplash;
  get pa_BomId(): string;
...
export interface IP_part_Backsplash_PartVarsWritable extends IP_part_Backsplash_PartVarsReadOnly {
...
export class P_part_Backsplash extends PartBase implements IP_part_Backsplash_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Backsplash, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Backsplash', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_Backsplash; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_Backsplash_ValueScript
    if (this.#pa_BomId === undefined) {
      this.pa_BomId = 'Backsplash01';
    }
    if (this.#pa_BtmColor === undefined) {
      this.pa_BtmColor = this.parent.mod_BacksplashColor;
    }
    if (this.#pa_EdgeBackColor === undefined) {
      this.pa_EdgeBackColor = this.parent.mod_BacksplashColor;
    }
    if (this.#pa_EdgeBackType === undefined) {
      this.pa_EdgeBackType = 'FRE';
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      this.pa_EdgeFrontColor = this.parent.mod_BacksplashColor;
    }
    ...
    if (this.#pa_EdgeLeftColor === undefined) {
      this.pa_EdgeLeftColor = this.parent.mod_BacksplashColor;
    }
    if (this.#pa_EdgeLeftType === undefined) {
      this.pa_EdgeLeftType = 'FRE';
    }
    if (this.#pa_EdgeRightColor === undefined) {
      this.pa_EdgeRightColor = this.parent.mod_BacksplashColor;
    }
    ...
    if (this.#pa_TopColor === undefined) {
      this.pa_TopColor = this.parent.mod_BacksplashColor;
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Backsplash (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Backsplash (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM182 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Backsplash_PartVarsReadOnly) {
            const part = partSelf;
...
GlobalFunc.process_BoardBom(this, part, elementType, elementCategory, parentBomId)
        ...
        _bom._description = "Output board parts";
        _bom.set(partSelf);

...
export interface cti_tab_PullOutMapping {
  readonly in_PullOutType? : string;
  readonly in_PullOutDesign? : string;
  readonly in_PullOutElementColor? : string;
  readonly in_PullOutConnectionPosition? : string;
  readonly in_PullOutOpeningType? : string;
}

export interface ctm_tab_PullOutMapping {
}

export interface cto_tab_PullOutMapping extends ctm_tab_PullOutMapping {
  readonly _id : number;
  readonly Object? : string;
  readonly PullOutPushToOpen? : boolean;
}

export interface ICT_tab_PullOutMapping
extends cti_tab_PullOutMapping, cto_tab_PullOutMapping {}

export class ct2_tab_PullOutMapping {

public findExactly(
     in_PullOutType: string|undefined,
     in_PullOutDesign: string|undefined,
     in_PullOutElementColor: string|undefined,
     in_PullOutConnectionPosition: string|undefined,
     in_PullOutOpeningType: string|undefined,
): cto_tab_PullOutMapping | undefined {
  const res = ct_tab_PullOutMapping.find((p) => 
p.in_PullOutType === in_PullOutType
 && p.in_PullOutDesign === in_PullOutDesign
 && p.in_PullOutElementColor === in_PullOutElementColor
 && p.in_PullOutConnectionPosition === in_PullOutConnectionPosition
 && p.in_PullOutOpeningType === in_PullOutOpeningType
);
return res;
}

public find(
predicate: (value: cti_tab_PullOutMapping) => boolean
): cto_tab_PullOutMapping | undefined {
  for (let index = 0; index < ct_tab_PullOutMapping.length; index++){
    const element = ct_tab_PullOutMapping[index];
...
export var ct_tab_PullOutMapping :ICT_tab_PullOutMapping[] = [
];

...
export interface IP_part_UprightGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Upright01;
  get pa_BomId(): string;
}

export interface IP_part_UprightGroup_PartVarsWritable extends IP_part_UprightGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_UprightGroup extends PartBase implements IP_part_UprightGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Upright01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_UprightGroup', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'YZX';
    this._noNCData = true;
    this._hidden = true;
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_UprightGroup (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 187
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM187 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_UprightGroup_PartVarsReadOnly) {
            const part = partSelf;

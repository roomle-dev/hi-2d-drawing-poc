...
//#region Imports
import { cbp_mc_CornerFillerFront01, dc_mc_CornerFillerFront01, adc_mc_CornerFillerFront01 } from '../internal/modules/mc_CornerFillerFront01'
...
// CUSTOMSCRIPT_mc_CornerFillerFront01_CREATEBUILDPLAN
  ...
  if (this.mod_FrontId == 'CornerFiller') {
    ...
    let retDoorInfo = GlobalFunc.find_FrontConstruction(this.mod_FrontProgram, this.mod_FrontDesign, this.mod_FrontId, this.mod_FrontWidth, this.mod_FrontHeight, this.mod_HandleDesign_matrix.HandleDesignGroup!, handlePosType);
    ...
    fillerHardwareInfo.GraphicItem.forEach(item => {
      ...
      graphicItem.forEach(graphic => {
        ...
        if (this.mod_Direction == "Right") {
          posxDrawing = retDoorInfo.WidthPos(this) + fillerHardwareInfo.PosX[i] + positionFillerHardware.InsertionPointX + positionFillerHardware.OffsetX;
        }
        else { 
        ...
        if (this.mod_Direction == "Right") {
          ...
          poszDrawing = retDoorInfo.DepthPos(this) + + positionFillerHardware.InsertionPointZ + positionFillerHardware.OffsetZ - positionFillerHardware.OversizeZ;
        }
        ...
        fillerLeftHardwareInfo.PosX.push(posxDrawing - retDoorInfo.WidthPos(this));
        ...
        switch (fillerHardwareInfo.ConnectionSide[i]) {
          case 'Right':
            fillerProcessing = this.addpart_FillerHardwareProcessing(retDoorInfo.WidthPos(this) + fillerHardwareInfo.PosX[i] - 1, fillerHardwareInfo.PosY[i], poszDrawing, 1, 1, dimzDrawing + positionFillerHardware.OversizeZ);
            break;
          case 'Left':
            if (this.mod_Direction == "Right") {
              ...
              fillerProcessing = this.addpart_FillerHardwareProcessing(fillerHardwareInfo.PosX[i]-1, fillerHardwareInfo.PosY[i], poszDrawing, 1, 1, dimzDrawing + positionFillerHardware.OversizeZ);
            }
            break
          case 'Top':
...
//
// Description:
// CreateAfterDataCompletion of mc_CornerunitStraightFiller01
// Copied from mc_Door01 --> The adjustments are marqued with the comment ////////////////////////////ADJUSTMENT FROM mc_Door01 TO mc_mc_CornerunitStraightFiller01
// Revisions:
//
//===================================================
...
//===================================================
  ...
  let Door: any;
  let FillerDesign = this.mod_FrontDesign == 'Automatic' ? retDoorInfo.FrontDesign! : this.mod_FrontDesign;

// Add the module
//---------------------------------------------------

if(FillerDesign == "FlatPanel") {  
  Door = this.addOD_M_mc_FrontPanel01(); 
}
else if (FillerDesign == "WoodFrame"){
  Door = this.addOD_M_mc_PanelWoodFrame01();
}
else if (FillerDesign == "AlluminiumFrame"){
  Door = this.addOD_M_mc_MetalFrame01();
}
else if (FillerDesign == "ThermoformedFrame"){
...
////////////////////////////ADJUSTMENT FROM mc_Door01 TO mc_mc_CornerunitStraightFiller01
//Door.mod_FrontType = "DoorLeft";
//if (this.mod_DoorDirection == 'Right'){Door.mod_FrontType = "DoorRight"}
Door.mod_FrontType = this.mod_FrontId + "Left";
if (this.mod_DoorDirection == 'Right'){Door.mod_FrontType = this.mod_FrontId + "Right"}
...
else if (this.mod_HandleDesign_matrix.FillerRelevant == true && this.mod_HandleDesign_matrix.HandleType == "Handle") 
{
  ...
  Handles.mod_Width = this.mod_FrontWidth - this.mod_FrontGapVert;
  Handles.mod_Height = this.mod_FrontHeight - this.mod_FrontGapHor;
  Handles.mod_Depth = this.mod_FrontThk;

  // Provide Information about part on which the handle is positioned
  Handles.mod_PartInfo = 'part_DoorLeft';
  if(this.mod_DoorDirection == 'Right'){Handles.mod_PartInfo = 'part_DoorRight'}

  // setOrigin
  Handles.setOrigin(this.mod_FrontGapVert / 2, 0, this.mod_FrontThk);
}
  ...
  if (this.mod_FrontId == 'CornerFiller') {
    ...
    if (this.mod_Direction == 'Right') {
      fillerHardwareInfo = GlobalFunc.process_FillerHardware(this, retDoorInfo.Width(this), retDoorInfo.Height(this), this.mod_Originpos[0] + retDoorInfo.WidthPos(this), this.mod_Originpos[1] + this.mod_FrontPosStart + retDoorInfo.HeightPos(this), 'FromFront');
    }
    else { 
      fillerHardwareInfo = GlobalFunc.process_FillerHardware(this, retDoorInfo.Width(this), retDoorInfo.Height(this), this.mod_Originpos[2] + retDoorInfo.WidthPos(this), this.mod_Originpos[1] + this.mod_FrontPosStart + retDoorInfo.HeightPos(this), 'FromRight');
    }

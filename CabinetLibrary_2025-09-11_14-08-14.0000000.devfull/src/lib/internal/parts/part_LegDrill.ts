...
export interface IP_part_LegDrill_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_LegDrill_PartVarsWritable extends IP_part_LegDrill_PartVarsReadOnly {
  set pa_Model3DGroupName(value: string);
  set pa_Rotation(value: number);
  set pa_ProcessingId(value: string);
}

export class P_part_LegDrill extends PartBase implements IP_part_LegDrill_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Leg01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_LegDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_LegDrill (id: ' + this._id + ')');
    }
    if (this.#pa_Rotation === undefined) {
      logError('Attribute pa_Rotation not set in part part_LegDrill (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_LegDrill (id: ' + this._id + ')');
    }

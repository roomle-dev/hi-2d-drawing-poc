...
export interface IP_part_PushtoopenBom_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Pushtoopen01;
  get pa_HardwareId(): string;
}

export interface IP_part_PushtoopenBom_PartVarsWritable extends IP_part_PushtoopenBom_PartVarsReadOnly {
  set pa_HardwareId(value: string);
}

export class P_part_PushtoopenBom extends PartBase implements IP_part_PushtoopenBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Pushtoopen01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_PushtoopenBom', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_PushtoopenBom_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_PushtoopenBom (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM192 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_PushtoopenBom_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
	  ...
	  let BOMName = GlobalFunc.find_PartSettings("part_PushToOpen");
	  ...
	  let tmpQty = info.BomQty;
        ...
        _bom._description = "BOM Push To Open";
        _bom.set(partSelf);

...
import { IPartParentsNonNull_mc_FlipliftHardware01 } from '../mod-interfaces'

export interface IP_part_FlipliftHardwareBom_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_FlipliftHardware01;
  get pa_BomId(): string;
}

export interface IP_part_FlipliftHardwareBom_PartVarsWritable extends IP_part_FlipliftHardwareBom_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_FlipliftHardwareBom extends PartBase implements IP_part_FlipliftHardwareBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FlipliftHardware01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FlipliftHardwareBom', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_FlipliftHardware01; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FlipliftHardwareBom_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FlipliftHardwareBom (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM199 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FlipliftHardwareBom_PartVarsReadOnly) {
            const part = partSelf;
...
try {
  ...
  // Catch the errors
    ...
    return
        ...
        _bom._description = "Hardware for fliplifts";
        _bom.set(partSelf);

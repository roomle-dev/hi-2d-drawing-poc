...
import { IPartParentsNonNull_mc_Handlestrip01 } from '../mod-interfaces'

export interface IP_part_Handlestrip_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Handlestrip01;
  get pa_Supplier(): string;
  get pa_SupplierArticle(): string;
  get pa_HardwareId(): string;
}

export interface IP_part_Handlestrip_PartVarsWritable extends IP_part_Handlestrip_PartVarsReadOnly {
  set pa_Supplier(value: string);
  set pa_SupplierArticle(value: string);
  set pa_HardwareId(value: string);
}

export class P_part_Handlestrip extends PartBase implements IP_part_Handlestrip_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Handlestrip01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Handlestrip', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_Handlestrip01; // module which created this part
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('pa_Supplier', this.#pa_Supplier);
  res.set('pa_SupplierArticle', this.#pa_SupplierArticle);
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_Handlestrip_ValueScript
    if (this.#pa_HardwareId === undefined) {
      this.pa_HardwareId = this.parent.mod_HardwareId;
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_Supplier === undefined) {
      logError('Attribute pa_Supplier not set in part part_Handlestrip (id: ' + this._id + ')');
    }
    if (this.#pa_SupplierArticle === undefined) {
      logError('Attribute pa_SupplierArticle not set in part part_Handlestrip (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_Handlestrip (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM153 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Handlestrip_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Add the handlestrip to BOM";
        _bom.set(partSelf);
  ...
  get pa_Supplier(): string { return this.#pa_Supplier!; }
  set pa_Supplier(val:string) { this.#pa_Supplier = val; }
  #pa_Supplier?: string;
  get pa_SupplierArticle(): string { return this.#pa_SupplierArticle!; }
  set pa_SupplierArticle(val:string) { this.#pa_SupplierArticle = val; }
  #pa_SupplierArticle?: string;

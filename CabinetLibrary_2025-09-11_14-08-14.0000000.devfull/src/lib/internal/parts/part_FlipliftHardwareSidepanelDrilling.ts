...
export interface IP_part_FlipliftHardwareSidepanelDrilling_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_FlipliftHardware01;
  get pa_ProcessingId(): string;
}

export interface IP_part_FlipliftHardwareSidepanelDrilling_PartVarsWritable extends IP_part_FlipliftHardwareSidepanelDrilling_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
}

export class P_part_FlipliftHardwareSidepanelDrilling extends PartBase implements IP_part_FlipliftHardwareSidepanelDrilling_PartVarsWritable {
...
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FlipliftHardwareSidepanelDrilling', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FlipliftHardwareSidepanelDrilling_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_FlipliftHardwareSidepanelDrilling (id: ' + this._id + ')');
    }

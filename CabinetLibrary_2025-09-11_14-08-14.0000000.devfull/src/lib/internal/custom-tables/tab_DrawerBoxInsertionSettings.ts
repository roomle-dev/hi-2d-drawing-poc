...
import { ITableParents_mc_Drawer01 } from '../var-interfaces';

export interface cti_tab_DrawerBoxInsertionSettings {
  ...
  readonly in_ShelffixedBtm? : boolean;
  readonly in_FrontStartPosMin : number;
  readonly in_FrontStartPosMax : number;
}

export interface ctm_tab_DrawerBoxInsertionSettings {
}

export interface cto_tab_DrawerBoxInsertionSettings extends ctm_tab_DrawerBoxInsertionSettings {
  readonly _id : number;
  DrawerBoxPos(m: ITableParents_mc_Drawer01, CarcaseFreeSpaceHeightStartPosition: number):number;
}

export interface ICT_tab_DrawerBoxInsertionSettings
extends cti_tab_DrawerBoxInsertionSettings, cto_tab_DrawerBoxInsertionSettings {}

export class ct2_tab_DrawerBoxInsertionSettings {
...
public findExactly(
     ...
     in_ShelffixedBtm: boolean|undefined,
     in_FrontStartPosMin: number,
     in_FrontStartPosMax: number,
): cto_tab_DrawerBoxInsertionSettings | undefined {
  const res = ct_tab_DrawerBoxInsertionSettings.find((p) => 
p.in_TypeElement === in_TypeElement
 ...
 && p.in_ShelffixedBtm === in_ShelffixedBtm
 && p.in_FrontStartPosMin === in_FrontStartPosMin
 && p.in_FrontStartPosMax === in_FrontStartPosMax
);
return res;
}

public find(
predicate: (value: cti_tab_DrawerBoxInsertionSettings) => boolean
): cto_tab_DrawerBoxInsertionSettings | undefined {
  for (let index = 0; index < ct_tab_DrawerBoxInsertionSettings.length; index++){
    const element = ct_tab_DrawerBoxInsertionSettings[index];
...
export var ct_tab_DrawerBoxInsertionSettings :ICT_tab_DrawerBoxInsertionSettings[] = [
{
 _id: 4,
 in_TypeElement: "All",
 in_FrontProgram: "All",
 ...
 in_ShelffixedBtm: false,
 in_FrontStartPosMin: 0,
 in_FrontStartPosMax: 20,
  DrawerBoxPos(m: ITableParents_mc_Drawer01, CarcaseFreeSpaceHeightStartPosition: number):number {
    return CarcaseFreeSpaceHeightStartPosition +  3 - m.mod_Originpos[1];
  }
...
,{
 ...
 in_HandleDesign: "All",
 in_HandlePosType: "All",
 in_ShelffixedBtm: true,
 in_FrontStartPosMin: 20.1,
 in_FrontStartPosMax: 9999,
  DrawerBoxPos(m: ITableParents_mc_Drawer01, CarcaseFreeSpaceHeightStartPosition: number):number {
    return CarcaseFreeSpaceHeightStartPosition +  3 - m.mod_Originpos[1];
  }

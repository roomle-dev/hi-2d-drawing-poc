...
export interface cti_tab_PullOutHardwareInsertionSettings {
  readonly in_TypeElement? : string;
  readonly in_FrontProgram? : string;
  readonly in_HandelDesign? : string;
...
export interface ctm_tab_PullOutHardwareInsertionSettings {
}

export interface cto_tab_PullOutHardwareInsertionSettings extends ctm_tab_PullOutHardwareInsertionSettings {
  readonly _id : number;
  PullOutHardwarePos(m: any /* WARNING: No parent found; assuming 'any' */):number;
}

export interface ICT_tab_PullOutHardwareInsertionSettings
extends cti_tab_PullOutHardwareInsertionSettings, cto_tab_PullOutHardwareInsertionSettings {}

export class ct2_tab_PullOutHardwareInsertionSettings {

public findExactly(
     in_TypeElement: string|undefined,
     in_FrontProgram: string|undefined,
     in_HandelDesign: string|undefined,
     in_HandlePosType: string|undefined,
     in_ShelffixedBtm: boolean|undefined,
     in_FrontStartPosMin: number,
     in_FrontStartPosMax: number,
): cto_tab_PullOutHardwareInsertionSettings | undefined {
  const res = ct_tab_PullOutHardwareInsertionSettings.find((p) => 
p.in_TypeElement === in_TypeElement
 && p.in_FrontProgram === in_FrontProgram
 && p.in_HandelDesign === in_HandelDesign
...
public find(
predicate: (value: cti_tab_PullOutHardwareInsertionSettings) => boolean
): cto_tab_PullOutHardwareInsertionSettings | undefined {
  for (let index = 0; index < ct_tab_PullOutHardwareInsertionSettings.length; index++){
    const element = ct_tab_PullOutHardwareInsertionSettings[index];
...
export var ct_tab_PullOutHardwareInsertionSettings :ICT_tab_PullOutHardwareInsertionSettings[] = [
];

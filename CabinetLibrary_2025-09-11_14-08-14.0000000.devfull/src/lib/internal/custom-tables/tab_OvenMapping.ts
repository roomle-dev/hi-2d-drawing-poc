...
export interface cti_tab_OvenMapping {
  readonly in_Supplier? : string;
  readonly in_OvenId? : string;
}

export interface ctm_tab_OvenMapping {
}

export interface cto_tab_OvenMapping extends ctm_tab_OvenMapping {
  readonly _id : number;
  readonly ConstructionId? : string;
  readonly GraphicId? : string;
}

export interface ICT_tab_OvenMapping
extends cti_tab_OvenMapping, cto_tab_OvenMapping {}

export class ct2_tab_OvenMapping {

public findExactly(
     in_Supplier: string|undefined,
     in_OvenId: string|undefined,
): cto_tab_OvenMapping | undefined {
  const res = ct_tab_OvenMapping.find((p) => 
p.in_Supplier === in_Supplier
 && p.in_OvenId === in_OvenId
);
return res;
}

public find(
predicate: (value: cti_tab_OvenMapping) => boolean
): cto_tab_OvenMapping | undefined {
  for (let index = 0; index < ct_tab_OvenMapping.length; index++){
    const element = ct_tab_OvenMapping[index];
...
export var ct_tab_OvenMapping :ICT_tab_OvenMapping[] = [
{
 _id: 1,
 in_Supplier: "Siemens",
 in_OvenId: "BI510CNR0",
 ConstructionId: "BI510CNR0",
 GraphicId: "BI510CNR0"
}
,{
 _id: 2,
...
,{
 ...
 in_Supplier: "None",
 in_OvenId: "None",
 ConstructionId: "OvenDefault",
 GraphicId: "OvenDefault"
}
];

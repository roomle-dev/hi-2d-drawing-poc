...
export interface IP_part_HingeCarcase_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Hinge01;
  get pa_HardwareId(): string;
  get pa_TopColor(): string;
}

export interface IP_part_HingeCarcase_PartVarsWritable extends IP_part_HingeCarcase_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_TopColor(value: string);
}

export class P_part_HingeCarcase extends PartBase implements IP_part_HingeCarcase_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Hinge01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_HingeCarcase', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_HingeCarcase (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_HingeCarcase (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM159 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_HingeCarcase_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  let BOMName = GlobalFunc.find_PartSettings("part_HingeCarcase");
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Hinge Carcase BOM";
        _bom.set(partSelf);

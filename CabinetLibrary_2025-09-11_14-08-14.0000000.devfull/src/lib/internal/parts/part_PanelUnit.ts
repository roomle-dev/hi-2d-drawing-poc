...
export interface IP_part_PanelUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Panel01;
}

export interface IP_part_PanelUnit_PartVarsWritable extends IP_part_PanelUnit_PartVarsReadOnly {
}

export class P_part_PanelUnit extends PartBase implements IP_part_PanelUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Panel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_PanelUnit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }

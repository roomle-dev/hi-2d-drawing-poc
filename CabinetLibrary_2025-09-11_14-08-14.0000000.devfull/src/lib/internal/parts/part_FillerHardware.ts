...
export interface IP_part_FillerHardware_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01;
  get pa_HardwareId(): string;
}

export interface IP_part_FillerHardware_PartVarsWritable extends IP_part_FillerHardware_PartVarsReadOnly {
  set pa_HardwareId(value: string);
}

export class P_part_FillerHardware extends PartBase implements IP_part_FillerHardware_PartVarsWritable {
...
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FillerHardware', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FillerHardware_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_FillerHardware (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM219 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FillerHardware_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  let BOMName = GlobalFunc.find_PartSettings("part_Filler");
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "BOM for filler Hardware";
        _bom.set(partSelf);

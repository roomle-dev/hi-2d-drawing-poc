...
//#region Imports
import { cbp_mf_Fridge, dc_mf_Fridge, adc_mf_Fridge } from '../internal/modules/mf_Fridge'
import { GlobalFunc } from '../internal/global-func'
import { dc_mc_Door01, OD_M_mc_Door01 } from '../internal/modules/mc_Door01'
import { dc_mc_Fridge01, OD_M_mc_Fridge01 } from '../internal/modules/mc_Fridge01'
...
export function mf_Fridge_createBuildPlan(this: cbp_mf_Fridge):void {
  internal_enterModuleCreateBuildPlan('mf_Fridge', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mf_Fridge_CREATEBUILDPLAN
...
// ---------------------------------------------------------------
export function mf_Fridge_afterDataCompletion(this: adc_mf_Fridge):void {
  internal_enterModuleAfterDataCompletion('mf_Fridge', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mf_Fridge_AFTERDATACOMPLETION
	...
	function AddDoor(m: any, doorHeight: number, doorPosition: number, intNmbr: number) {
		...
		try {
			...
			if (doorInfo.configValid) {
				...
				Door.mod_FrontPosStart = m.mod_Originpos[1] + doorPosition;
	...
	if (FridgeInfo.SplitDoor) {
		...
		// Add the doors
		AddDoor(this, frontHgt1, 0, 1);
		AddDoor(this, frontHgt2, frontPos2, 2);
	}
	else {
...
// ---------------------------------------------------------------
export function mf_Fridge_manufacturerDataCompletion(this: dc_mf_Fridge):void {
  internal_enterModuleManufacturerDataCompletion('mf_Fridge', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mf_Fridge_MANUFACTURERDATACOMPLETION

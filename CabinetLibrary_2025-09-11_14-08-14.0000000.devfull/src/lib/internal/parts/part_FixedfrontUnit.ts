...
import { IPartParentsNonNull_mc_Fixedfront01 } from '../mod-interfaces'

export interface IP_part_FixedfrontUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Fixedfront01;
  get pa_BomId(): string;
  get pa_PartgroupBomId(): string;
}

export interface IP_part_FixedfrontUnit_PartVarsWritable extends IP_part_FixedfrontUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
  set pa_PartgroupBomId(value: string);
}

export class P_part_FixedfrontUnit extends PartBase implements IP_part_FixedfrontUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Fixedfront01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FixedfrontUnit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Fixedfront01; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FixedfrontUnit_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FixedfrontUnit (id: ' + this._id + ')');
    }
    if (this.#pa_PartgroupBomId === undefined) {
      logError('Attribute pa_PartgroupBomId not set in part part_FixedfrontUnit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 208
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM208 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FixedfrontUnit_PartVarsReadOnly) {
            const part = partSelf;
...
// Type of the Bom-Element (will be checked in order output)
  ...
  this.bom_ElementCategory = elementCategory
        ...
        _bom._description = "Partgroup fixed front";
        _bom.set(partSelf);

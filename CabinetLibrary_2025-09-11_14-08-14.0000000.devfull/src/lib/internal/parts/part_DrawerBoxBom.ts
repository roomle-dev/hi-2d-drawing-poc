...
export interface IP_part_DrawerBoxBom_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_DrawerBox01;
  get pa_HardwareId(): string;
  get pa_HardwareId2(): string;
  get pa_HardwareId3(): string;
  get pa_HardwareId4(): string;
}

export interface IP_part_DrawerBoxBom_PartVarsWritable extends IP_part_DrawerBoxBom_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_HardwareId2(value: string);
  set pa_HardwareId3(value: string);
  set pa_HardwareId4(value: string);
}

export class P_part_DrawerBoxBom extends PartBase implements IP_part_DrawerBoxBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerBoxBom', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  ...
  res.set('pa_HardwareId2', this.#pa_HardwareId2);
  res.set('pa_HardwareId3', this.#pa_HardwareId3);
  res.set('pa_HardwareId4', this.#pa_HardwareId4);
    return res;
  }
  setAttributes() {
    try {
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareId2 === undefined) {
      logError('Attribute pa_HardwareId2 not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareId3 === undefined) {
      logError('Attribute pa_HardwareId3 not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareId4 === undefined) {
      logError('Attribute pa_HardwareId4 not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM177 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_DrawerBoxBom_PartVarsReadOnly) {
            const part = partSelf;
...
{  
  ...
  HardwareElem.bom_Thk = HardwareInfo.Thickness;
        ...
        _bom._description = "All the hardware components for the drawer box";
        _bom.set(partSelf);
  ...
  get pa_HardwareId2(): string { return this.#pa_HardwareId2!; }
  set pa_HardwareId2(val:string) { this.#pa_HardwareId2 = val; }
  #pa_HardwareId2?: string;
  get pa_HardwareId3(): string { return this.#pa_HardwareId3!; }
  set pa_HardwareId3(val:string) { this.#pa_HardwareId3 = val; }
  #pa_HardwareId3?: string;
  get pa_HardwareId4(): string { return this.#pa_HardwareId4!; }
  set pa_HardwareId4(val:string) { this.#pa_HardwareId4 = val; }
  #pa_HardwareId4?: string;
}

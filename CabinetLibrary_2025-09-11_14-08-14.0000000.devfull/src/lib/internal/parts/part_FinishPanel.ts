...
import { IPartParentsNonNull_mc_Upright01 } from '../mod-interfaces'

export interface IP_part_FinishPanel_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Upright01;
  get pa_FinishPanelThk(): number;
  get pa_FinishPanelDepth(): number;
  get pa_FinishPanelHeight(): number;
  get pa_BomId(): string;
...
export interface IP_part_FinishPanel_PartVarsWritable extends IP_part_FinishPanel_PartVarsReadOnly {
  set pa_FinishPanelThk(value: number);
  set pa_FinishPanelDepth(value: number);
  set pa_FinishPanelHeight(value: number);
...
export class P_part_FinishPanel extends PartBase implements IP_part_FinishPanel_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Upright01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FinishPanel', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'ZYX';
  }
  parent: IPartParentsNonNull_mc_Upright01; // module which created this part
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('pa_FinishPanelThk', this.#pa_FinishPanelThk);
  res.set('pa_FinishPanelDepth', this.#pa_FinishPanelDepth);
  res.set('pa_FinishPanelHeight', this.#pa_FinishPanelHeight);
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FinishPanel_ValueScript
    if (this.#pa_BtmColor === undefined) {
      this.pa_BtmColor = this.parent.mod_UprightColor;
    }
    if (this.#pa_EdgeBackColor === undefined) {
      this.pa_EdgeBackColor = this.parent.mod_UprightColor;
    }
    if (this.#pa_EdgeBackType === undefined) {
      this.pa_EdgeBackType = 'FRE';
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      this.pa_EdgeFrontColor = this.parent.mod_UprightColor;
    }
    ...
    if (this.#pa_TopColor === undefined) {
      this.pa_TopColor = this.parent.mod_UprightColor;
    }
    if (this.#pa_Program === undefined) {
      this.pa_Program = 'DemoProgram1';
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      this.pa_EdgeLeftColor = this.parent.mod_UprightColor;
    }
    if (this.#pa_EdgeLeftType === undefined) {
      this.pa_EdgeLeftType = 'FRE';
    }
    if (this.#pa_EdgeRightColor === undefined) {
      this.pa_EdgeRightColor = this.parent.mod_UprightColor;
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_FinishPanelThk === undefined) {
      logError('Attribute pa_FinishPanelThk not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_FinishPanelDepth === undefined) {
      logError('Attribute pa_FinishPanelDepth not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_FinishPanelHeight === undefined) {
      logError('Attribute pa_FinishPanelHeight not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_FinishPanel (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM186 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FinishPanel_PartVarsReadOnly) {
            const part = partSelf;
...
GlobalFunc.process_BoardBom(this, part, elementType, elementCategory, parentBomId)
  ...
  get pa_FinishPanelThk(): number { return this.#pa_FinishPanelThk!; }
  set pa_FinishPanelThk(val:number) { this.#pa_FinishPanelThk = val; }
  #pa_FinishPanelThk?: number;
  get pa_FinishPanelDepth(): number { return this.#pa_FinishPanelDepth!; }
  set pa_FinishPanelDepth(val:number) { this.#pa_FinishPanelDepth = val; }
  #pa_FinishPanelDepth?: number;
  get pa_FinishPanelHeight(): number { return this.#pa_FinishPanelHeight!; }
  set pa_FinishPanelHeight(val:number) { this.#pa_FinishPanelHeight = val; }
  #pa_FinishPanelHeight?: number;

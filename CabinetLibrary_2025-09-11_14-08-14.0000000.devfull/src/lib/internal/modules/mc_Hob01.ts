...
import { IModParents_mf_Hob } from '../mod-interfaces'
import { IP_part_CountertopCutout_PartVarsWritable, P_part_CountertopCutout } from '../parts/part_CountertopCutout'
import { OD_M_mc_ApplianceGraphic, dc_mc_ApplianceGraphic } from './mc_ApplianceGraphic'
import { IModuleNonNull_mc_Hob01,IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood,IModParents_mc_Hob01,IPartParentsNonNull_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood,IPartParentsNonNull_mc_Hob01} from '../mod-interfaces'
import {IModVar_mod_HobGraphicId,IModVarNonNull_mod_HobGraphicId,IModVar_mod_HobConstructionId,IModVarNonNull_mod_HobConstructionId} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_HobGraphicId, IMatrix_mod_HobConstructionId} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_CountertopCutout } from '../part-add-interfaces';
import { mc_Hob01_createBuildPlan, mc_Hob01_afterDataCompletion, mc_Hob01_manufacturerDataCompletion } from '../../modules/mc_Hob01';

export interface cbp_mc_Hob01 extends IModBaseProp
, IPartAdd_part_CountertopCutout, IModVarNonNull_mod_HobGraphicId, IModVarNonNull_mod_HobConstructionId {
  parent: IModParents_mf_Hob;
...
export interface dc_mc_Hob01 extends IModBaseProp
, IModVar_mod_HobGraphicId, IModVar_mod_HobConstructionId {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mc_Hob01;
  parent: IModParents_mf_Hob;
  addOD_M_mc_ApplianceGraphic(index?: number): dc_mc_ApplianceGraphic;
}

export interface adc_mc_Hob01 extends IModBaseProp
...
export class OD_M_mc_Hob01 extends OD_Base  implements dc_mc_Hob01
,IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood,IModParents_mc_Hob01
,IModVar_mod_HobGraphicId,IModVar_mod_HobConstructionId
 {
  constructor(parent: IModParents_mf_Hob, manufacturerMode?:boolean) {
    super('mc_Hob01', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_Hob01_NonNull(this);
  }
  parent: IModParents_mf_Hob;
  _selfNonNull: OD_M_mc_Hob01_NonNull;

  override variants:string[] = ['mod_HobGraphicId','mod_HobConstructionId',];
  #mod_HobGraphicId?: string;
  get mod_HobGraphicId(): string | undefined { return this.#mod_HobGraphicId}
  set mod_HobGraphicId(value: string | undefined) {
    if(this._adcFinished){
      logError("The property 'mod_HobGraphicId' must not be changed after the module was completed with 'afterDataCompletion'");
      return;
    }
    if(this.#mod_HobGraphicId === value) return;
    this.#mod_HobGraphicId = value;
  }

  mod_HobGraphicId_matrix?: IMatrix_mod_HobGraphicId;
  #mod_HobConstructionId?: string;
  get mod_HobConstructionId(): string | undefined { return this.#mod_HobConstructionId}
  set mod_HobConstructionId(value: string | undefined) {
    if(this._adcFinished){
      logError("The property 'mod_HobConstructionId' must not be changed after the module was completed with 'afterDataCompletion'");
      return;
    }
    if(this.#mod_HobConstructionId === value) return;
    this.#mod_HobConstructionId = value;
  }

  mod_HobConstructionId_matrix?: IMatrix_mod_HobConstructionId;
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('mod_HobGraphicId', this.mod_HobGraphicId);
  res.set('mod_HobConstructionId', this.mod_HobConstructionId);
    return res;
  }
  override loadJson(json:any, withSubModules:boolean = true, manufacturerMode:boolean) {
    ...
    { internal_enterValidateVariant(this.modId, this._id, 'mod_HobGraphicId');
    const val = _toString(json['attributes']['mod_HobGraphicId'], 'mod_HobGraphicId');
    internal_leaveValidateVariant();
    this.mod_HobGraphicId = val; }
    { internal_enterValidateVariant(this.modId, this._id, 'mod_HobConstructionId');
    const val = _toString(json['attributes']['mod_HobConstructionId'], 'mod_HobConstructionId');
    internal_leaveValidateVariant();
    this.mod_HobConstructionId = val; }
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_HobGraphicId === undefined) {
      this.mod_HobGraphicId = "None";
    }
    if (this.mod_HobConstructionId === undefined) {
      this.mod_HobConstructionId = "None";
    }
  ...
  #manufacturerDataCompletionInternal = mc_Hob01_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_Hob01_NonNull(this);
  ...
  seal(): IModuleNonNull_mc_Hob01{
    ...
    const adc = new OD_M_mc_Hob01_NonNull(this);
    return adc;
  }
  ...
  override validateSelections():void {
    this.mod_HobGraphicId_matrix = VariantValidation.mod_HobGraphicId(this, this);
    this.mod_HobConstructionId_matrix = VariantValidation.mod_HobConstructionId(this, this);
  }
  addpart_CountertopCutout(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_CountertopCutout_PartVarsWritable {
    var c = new P_part_CountertopCutout(this._selfNonNull, x, y, z, dimx, dimy, dimz);
    this.p.push(c);
    return c;
  }

  override createBuildPlan():void {
    var cbp = new OD_M_mc_Hob01_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_Hob01_NonNull implements cbp_mc_Hob01, adc_mc_Hob01 ,IPartParentsNonNull_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood,IPartParentsNonNull_mc_Hob01 {
  constructor(parent: OD_M_mc_Hob01) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_Hob01;
  ...
  #createBuildPlanInternal = mc_Hob01_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_Hob01_afterDataCompletion;
  ...
  parent: IModParents_mf_Hob;
  addpart_CountertopCutout(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_CountertopCutout_PartVarsWritable {
    return this.#internalParent.addpart_CountertopCutout(x, y, z, dimx, dimy, dimz);
  }
  get mod_HobGraphicId(): string  {
    return this.#internalParent.mod_HobGraphicId!;
  }
  get mod_HobConstructionId(): string  {
    return this.#internalParent.mod_HobConstructionId!;
  }
  addOD_M_mc_ApplianceGraphic(index?: number): dc_mc_ApplianceGraphic {

...
//#region Imports
import { cbp_mc_Fixedfront01, dc_mc_Fixedfront01, adc_mc_Fixedfront01 } from '../internal/modules/mc_Fixedfront01'
...
export function mc_Fixedfront01_createBuildPlan(this: cbp_mc_Fixedfront01):void {
  internal_enterModuleCreateBuildPlan('mc_Fixedfront01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mc_Fixedfront01_CREATEBUILDPLAN
...
// ---------------------------------------------------------------
export function mc_Fixedfront01_afterDataCompletion(this: adc_mc_Fixedfront01):void {
  internal_enterModuleAfterDataCompletion('mc_Fixedfront01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_Fixedfront01_AFTERDATACOMPLETION
  ...
  let retFrontInfo = GlobalFunc.find_FrontConstruction(this.mod_FrontProgram, this.mod_FrontDesign, 'Drawer', this.mod_FrontWidth, this.mod_FrontHeight, this.mod_HandleDesign_matrix.HandleDesignGroup!, handlePosType);
  ...
  let Front: any;
  let DrawerDesign = retFrontInfo.FrontModuleId;
  ...
  Front.mod_Width = retFrontInfo.Width(this); 
  Front.mod_Height = retFrontInfo.Height(this); 
  Front.mod_Depth = retFrontInfo.Thickness;
  Front.mod_FrontType = "Fixedfront";

  // setOrigin
  Front.setOrigin(retFrontInfo.WidthPos(this), retFrontInfo.HeightPos(this), retFrontInfo.DepthPos(this)); 
  let SealedFront = Front.seal();
  frontWeight = SealedFront.mod_FrontpanelWeightCalculations[0];
...
// ---------------------------------------------------------------
export function mc_Fixedfront01_manufacturerDataCompletion(this: dc_mc_Fixedfront01):void {
  internal_enterModuleManufacturerDataCompletion('mc_Fixedfront01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_Fixedfront01_MANUFACTURERDATACOMPLETION

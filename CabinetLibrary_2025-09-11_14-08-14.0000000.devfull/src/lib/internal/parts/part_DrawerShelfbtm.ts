...
import { P_part_DrawerBackwallWood, IP_part_DrawerBackwallWood_PartVarsReadOnly } from './part_DrawerBackwallWood'
...
import { IPartParentsNonNull_mc_DrawerBox01 } from '../mod-interfaces'

export interface IP_part_DrawerShelfbtm_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_DrawerBox01;
  get pa_BomId(): string;
...
export interface IP_part_DrawerShelfbtm_PartVarsWritable extends IP_part_DrawerShelfbtm_PartVarsReadOnly {
...
export class P_part_DrawerShelfbtm extends PartBase implements IP_part_DrawerShelfbtm_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerShelfbtm', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XZY';
  }
  parent: IPartParentsNonNull_mc_DrawerBox01; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_DrawerShelfbtm_ValueScript
    ...
    if (this.#pa_ColorGrainGroup === undefined) {
      this.pa_ColorGrainGroup = 'NoGrain';
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_ProgramGrainGroup === undefined) {
      logError('Attribute pa_ProgramGrainGroup not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_ColorGrainGroup === undefined) {
      logError('Attribute pa_ColorGrainGroup not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_GrainLogic === undefined) {
      logError('Attribute pa_GrainLogic not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
    if (this.#pa_TypeElement === undefined) {
      logError('Attribute pa_TypeElement not set in part part_DrawerShelfbtm (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM179 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_DrawerShelfbtm_PartVarsReadOnly) {
            const part = partSelf;
            internal_enterBomPartMasterDataElements(partSelf._id, partSelf._partId);
             // ####################### CUSTOM SCRIPTS ########################
// Add the Board
            let Board = this.addbomout_Board();
            ...
            GlobalFunc.process_BoardBom(Board, part, elementType, elementCategory, parentBomId)
             // ################### END CUSTOM SCRIPTS ########################
            internal_leaveBomPartMasterDataElements();
          }
        }
        _bom._description = "Boards and edges for drawerbox bottom shelf";
        _bom.set(partSelf);
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 386: part_DrawerBoxDrilling Collision
    if (touch == PartTouch.Collision && partNext instanceof P_part_DrawerBoxDrilling) {
        let part2 = <P_part_DrawerBoxDrilling>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT386 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_DrawerShelfbtm_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerBoxDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
  ...
  drillings.forEach(x => 
    {  
      ...
      drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU/2 + '" /></svg>', 'y');
          ...
          _bom._description = "Processings forced from the drawer box";
      ...
      try {
      // ncout_Group: 388: part_DrawerBackwallWood FromTop
    if (touch == PartTouch.FromTop && partNext instanceof P_part_DrawerBackwallWood) {
        let part2 = <P_part_DrawerBackwallWood>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT388 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_DrawerShelfbtm_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerBackwallWood_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
for (let i = 1; i <= numberOfDrills; i++) {
  ...
  drilling02.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + 2 + '" /></svg>', 'y');
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Connection to drawer backwall wood from top";
          _bom.set(partSelf, touch, part2, posRel);
          partSelf._bom.push(_bom);
          if(_bom._subBom){
            _bom._subBom.forEach(p => partSelf._bom.push(p));
      ...
      try {
      // ncout_Group: 395: part_DrawerShelfbtmConnectorDrilling FromBottom
    if (touch == PartTouch.FromBottom && partNext instanceof P_part_DrawerShelfbtmConnectorDrilling) {
        let part2 = <P_part_DrawerShelfbtmConnectorDrilling>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT395 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_DrawerShelfbtm_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerShelfbtmConnectorDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
{  
  ...
  drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU/2 + '" /></svg>', 'y');
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Drawer box bottom connector to front";
          _bom.set(partSelf, touch, part2, posRel);
          partSelf._bom.push(_bom);
          if(_bom._subBom){
            _bom._subBom.forEach(p => partSelf._bom.push(p));

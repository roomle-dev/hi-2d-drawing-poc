...
import { IPartParentsNonNull_mc_Storageunit01 } from '../mod-interfaces'

export interface IP_part_Storageunit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Storageunit01;
  get pa_BomId(): string;
}

export interface IP_part_Storageunit_PartVarsWritable extends IP_part_Storageunit_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_Storageunit extends PartBase implements IP_part_Storageunit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Storageunit01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Storageunit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Storageunit01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Storageunit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 148
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM148 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Storageunit_PartVarsReadOnly) {
            const part = partSelf;

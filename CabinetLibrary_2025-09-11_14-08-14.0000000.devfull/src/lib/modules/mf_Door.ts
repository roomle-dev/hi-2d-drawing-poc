...
//#region Imports
import { cbp_mf_Door, dc_mf_Door, adc_mf_Door } from '../internal/modules/mf_Door'
...
export function mf_Door_createBuildPlan(this: cbp_mf_Door):void {
  internal_enterModuleCreateBuildPlan('mf_Door', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mf_Door_CREATEBUILDPLAN
...
// ---------------------------------------------------------------
export function mf_Door_afterDataCompletion(this: adc_mf_Door):void {
  internal_enterModuleAfterDataCompletion('mf_Door', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mf_Door_AFTERDATACOMPLETION
	...
	let VertDividerInfoList = JSON.parse(this.mod_VertDividerInfoList[0]);

	//Initialize variables
	let vertDividerType = VertDividerInfoList.Type;
	...
	try {
		...
		let doorInfo = GlobalFunc.process_Door(this);
		...
		if (doorInfo.configValid) {
			...
			for (let i = 1; i <= doorInfo.DoorQty; i++) {
				...
				// Add the module
				let Door = this.addOD_M_mc_Door01();
				...
				Door.mod_CarcasePartInfo.push(CarcasePartInfoPerFrontJson);
	...
	//===================================================
	//          Add module for the adjustable shelf
	//===================================================
...
// ---------------------------------------------------------------
export function mf_Door_manufacturerDataCompletion(this: dc_mf_Door):void {
  internal_enterModuleManufacturerDataCompletion('mf_Door', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mf_Door_MANUFACTURERDATACOMPLETION

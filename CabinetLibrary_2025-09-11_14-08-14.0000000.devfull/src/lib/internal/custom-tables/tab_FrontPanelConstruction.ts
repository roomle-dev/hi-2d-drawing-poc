...
export interface cti_tab_FrontPanelConstruction {
  ...
  readonly in_GrainGroupId? : string;
}

export interface ctm_tab_FrontPanelConstruction {
}

export interface cto_tab_FrontPanelConstruction extends ctm_tab_FrontPanelConstruction {
  readonly _id : number;
...
extends cti_tab_FrontPanelConstruction, cto_tab_FrontPanelConstruction {}

export class ct2_tab_FrontPanelConstruction {

public findExactly(
     in_PartGroup: string|undefined,
     in_FrontConstructionId: string|undefined,
     in_GrainGroupId: string|undefined,
): cto_tab_FrontPanelConstruction | undefined {
  const res = ct_tab_FrontPanelConstruction.find((p) => 
p.in_PartGroup === in_PartGroup
 && p.in_FrontConstructionId === in_FrontConstructionId
 && p.in_GrainGroupId === in_GrainGroupId
);
return res;
}

public find(
predicate: (value: cti_tab_FrontPanelConstruction) => boolean
): cto_tab_FrontPanelConstruction | undefined {
  for (let index = 0; index < ct_tab_FrontPanelConstruction.length; index++){
    const element = ct_tab_FrontPanelConstruction[index];

...
import { P_part_HeatshelfFront, IP_part_HeatshelfFront_PartVarsReadOnly } from './part_HeatshelfFront'
...
export interface IP_part_Heatshelf_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_Heatshelf_PartVarsWritable extends IP_part_Heatshelf_PartVarsReadOnly {
...
export class P_part_Heatshelf extends PartBase implements IP_part_Heatshelf_PartVarsWritable {
...
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Heatshelf', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XZY';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_TypeElement === undefined) {
      logError('Attribute pa_TypeElement not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_Heatshelf (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM211 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Heatshelf_PartVarsReadOnly) {
            const part = partSelf;
...
// Type of the Bom-Element (will be checked in order output)
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 539: part_HeatshelfFront FromTop
    if (touch == PartTouch.FromTop && partNext instanceof P_part_HeatshelfFront) {
        let part2 = <P_part_HeatshelfFront>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT539 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Heatshelf_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HeatshelfFront_PartVarsReadOnly, posRel:CollisionDataRel) {
...
// Schuler Consulting
  ...
  //
  // Description:
  // Connection between Heatshelf and HeatshelfFront
  // Touch FromTop
  //
  // Revisions:
  // 

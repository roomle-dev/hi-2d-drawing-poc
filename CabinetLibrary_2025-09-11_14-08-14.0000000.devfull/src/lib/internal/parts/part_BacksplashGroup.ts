...
export interface IP_part_BacksplashGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Backsplash;
  get pa_BomId(): string;
}

export interface IP_part_BacksplashGroup_PartVarsWritable extends IP_part_BacksplashGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_BacksplashGroup extends PartBase implements IP_part_BacksplashGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Backsplash, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_BacksplashGroup', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_BacksplashGroup_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_BacksplashGroup (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 183
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM183 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_BacksplashGroup_PartVarsReadOnly) {
            const part = partSelf;

...
export interface cti_tab_DuststripMapping {
  readonly in_FrontProgram? : string;
  readonly in_FrontColor? : string;
  readonly in_FrontDesign? : string;
}

export interface ctm_tab_DuststripMapping {
}

export interface cto_tab_DuststripMapping extends ctm_tab_DuststripMapping {
  readonly _id : number;
  readonly DuststripObject? : string;
}

export interface ICT_tab_DuststripMapping
extends cti_tab_DuststripMapping, cto_tab_DuststripMapping {}

export class ct2_tab_DuststripMapping {

public findExactly(
     in_FrontProgram: string|undefined,
     in_FrontColor: string|undefined,
     in_FrontDesign: string|undefined,
): cto_tab_DuststripMapping | undefined {
  const res = ct_tab_DuststripMapping.find((p) => 
...
public find(
predicate: (value: cti_tab_DuststripMapping) => boolean
): cto_tab_DuststripMapping | undefined {
  for (let index = 0; index < ct_tab_DuststripMapping.length; index++){
    const element = ct_tab_DuststripMapping[index];
...
export var ct_tab_DuststripMapping :ICT_tab_DuststripMapping[] = [
{
 _id: 3,
 in_FrontProgram: "All",
 in_FrontColor: "All",
 in_FrontDesign: "All",
 DuststripObject: "GlueDuststripGreyBrown"
}
,{
 _id: 4,
 in_FrontProgram: "All",
 in_FrontColor: "Demowhite",
 in_FrontDesign: "All",
 DuststripObject: "GlueDuststripWhite"
}
];

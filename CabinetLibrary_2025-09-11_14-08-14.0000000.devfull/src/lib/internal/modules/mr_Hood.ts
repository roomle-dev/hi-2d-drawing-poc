...
import { IModuleNonNull_mr_Hood,IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood,IPartParentsNonNull_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood} from '../mod-interfaces'
import {IModVar_mod_HoodSupplier,IModVarNonNull_mod_HoodSupplier,IModVar_mod_HoodId,IModVarNonNull_mod_HoodId,IModVar_mod_HeightPosInsertion,IModVarNonNull_mod_HeightPosInsertion} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_HoodSupplier, IMatrix_mod_HoodId, IMatrix_mod_HeightPosInsertion} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'
import { IGlobalVarsParent } from '../global-vars-parent'

import { mr_Hood_createBuildPlan, mr_Hood_afterDataCompletion, mr_Hood_manufacturerDataCompletion } from '../../modules/mr_Hood';

export interface cbp_mr_Hood extends IModBaseProp
...
export interface dc_mr_Hood extends IModBaseProp
, IModVar_mod_HoodSupplier, IModVar_mod_HoodId, IModVar_mod_HeightPosInsertion {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mr_Hood;
  addOD_M_mc_ApplianceGraphic(index?: number): dc_mc_ApplianceGraphic;
}

export interface adc_mr_Hood extends IModBaseProp
, IModVarNonNull_mod_HoodSupplier, IModVarNonNull_mod_HoodId, IModVarNonNull_mod_HeightPosInsertion {
  get m(): OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  addOD_M_mc_ApplianceGraphic(index?: number): dc_mc_ApplianceGraphic;
...
export class OD_M_mr_Hood extends OD_Base  implements dc_mr_Hood
...
,IModVar_mod_HoodSupplier,IModVar_mod_HoodId,IModVar_mod_HeightPosInsertion
 {
  constructor(parent?: OD_Base, manufacturerMode?:boolean) {
    super('mr_Hood', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this._g = new GlobalVars();
    this._openGroups = new Map<string, OpenGroup>();
    this._partGroups = new Map<string, PartGroup>();
    this._selfNonNull = new OD_M_mr_Hood_NonNull(this);
  }
  _selfNonNull: OD_M_mr_Hood_NonNull;
  ...
  override variants:string[] = ['mod_HoodSupplier','mod_HoodId','mod_HeightPosInsertion',];
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_HeightPosInsertion === undefined) {
      this.mod_HeightPosInsertion = 1500;
    }
  ...
  #manufacturerDataCompletionInternal = mr_Hood_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mr_Hood_NonNull(this);
  ...
  seal(): IModuleNonNull_mr_Hood{
    ...
    const adc = new OD_M_mr_Hood_NonNull(this);
    return adc;
  }
  ...
  override validateSelections():void {
    this.mod_HoodSupplier_matrix = VariantValidation.mod_HoodSupplier(this, this);
    this.mod_HoodId_matrix = VariantValidation.mod_HoodId(this, this);
    this.mod_HeightPosInsertion_matrix = VariantValidation.mod_HeightPosInsertion(this, this);
  }

  override createBuildPlan():void {
    var cbp = new OD_M_mr_Hood_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mr_Hood_NonNull implements cbp_mr_Hood, adc_mr_Hood ,IPartParentsNonNull_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood {
  constructor(parent: OD_M_mr_Hood) {
    this.#internalParent = parent;
  }
  #internalParent: OD_M_mr_Hood;
  ...
  #createBuildPlanInternal = mr_Hood_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mr_Hood_afterDataCompletion;

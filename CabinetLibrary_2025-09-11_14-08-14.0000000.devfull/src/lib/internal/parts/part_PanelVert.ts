...
import { IPartParentsNonNull_mc_Panel01 } from '../mod-interfaces'

export interface IP_part_PanelVert_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Panel01;
  get pa_GrainDirection(): string;
}

export interface IP_part_PanelVert_PartVarsWritable extends IP_part_PanelVert_PartVarsReadOnly {
  set pa_GrainDirection(value: string);
}

export class P_part_PanelVert extends PartBase implements IP_part_PanelVert_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Panel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_PanelVert', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_Panel01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_PanelVert (id: ' + this._id + ')');
    }

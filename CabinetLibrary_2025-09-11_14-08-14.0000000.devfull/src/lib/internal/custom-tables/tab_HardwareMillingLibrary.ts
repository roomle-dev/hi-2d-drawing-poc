...
export interface cti_tab_HardwareMillingLibrary {
  readonly in_ProcessingId? : string;
  readonly in_Part? : string;
}

export interface ctm_tab_HardwareMillingLibrary {
  readonly matrix_RD? : number;
  readonly matrix_WI? : number;
  readonly matrix_KO? : string;
  readonly matrix_T_? : string;
}

export interface cto_tab_HardwareMillingLibrary extends ctm_tab_HardwareMillingLibrary {
  readonly _id : number;
...
export interface ICT_tab_HardwareMillingLibrary
extends cti_tab_HardwareMillingLibrary, cto_tab_HardwareMillingLibrary {}

export class ct2_tab_HardwareMillingLibrary {

public findExactly(
     in_ProcessingId: string|undefined,
     in_Part: string|undefined,
): cto_tab_HardwareMillingLibrary | undefined {
  const res = ct_tab_HardwareMillingLibrary.find((p) => 
...
public find(
predicate: (value: cti_tab_HardwareMillingLibrary) => boolean
): cto_tab_HardwareMillingLibrary | undefined {
  for (let index = 0; index < ct_tab_HardwareMillingLibrary.length; index++){
    const element = ct_tab_HardwareMillingLibrary[index];

...
export interface IP_part_DrawerBackwallWood_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_DrawerBackwallWood_PartVarsWritable extends IP_part_DrawerBackwallWood_PartVarsReadOnly {
  ...
  set pa_ProcessingId(value: string);
...
export class P_part_DrawerBackwallWood extends PartBase implements IP_part_DrawerBackwallWood_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerBackwallWood', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  res.set('pa_ProcessingId', this.#pa_ProcessingId);
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_DrawerBackwallWood_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_GrainLogic === undefined) {
      logError('Attribute pa_GrainLogic not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_TypeElement === undefined) {
      logError('Attribute pa_TypeElement not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_ProgramGrainGroup === undefined) {
      logError('Attribute pa_ProgramGrainGroup not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
    if (this.#pa_ColorGrainGroup === undefined) {
      logError('Attribute pa_ColorGrainGroup not set in part part_DrawerBackwallWood (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM178 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_DrawerBackwallWood_PartVarsReadOnly) {
            const part = partSelf;
...
// Add the Board
        ...
        _bom._description = "Boards and edges for drawerbox backwall wood";
        _bom.set(partSelf);
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 396: part_DrawerBoxDrilling Collision
    if (touch == PartTouch.Collision && partNext instanceof P_part_DrawerBoxDrilling) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT396 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_DrawerBackwallWood_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerBoxDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
...
let processings = GlobalFunc.find_ProcessingMapping(partSelf.pa_ProcessingId);

// Find the drillings for the sidepanel
processings.forEach((processing) => {
  if(processing.ProcessingLibrary == 'DrillVertical'){
    let foundDrillings = GlobalFunc.find_HardwareDrillVertLibrary(processing.ProcessingId!, 'BoxBackwall');
    drillings = drillings.concat(foundDrillings);
  }
});

//====================================================================
// For each drilling (insert the drillings) left side
//====================================================================
...
//====================================================================
// For each drilling (insert the drillings) right side
//====================================================================
...
{  
  ...
  drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU/2 + '" /></svg>', 'z');
          ...
          _bom._description = "Drills in backwall for connection of drawer boxes";
  ...
  get pa_ProcessingId(): string { return this.#pa_ProcessingId!; }
  set pa_ProcessingId(val:string) { this.#pa_ProcessingId = val; }
  #pa_ProcessingId?: string;

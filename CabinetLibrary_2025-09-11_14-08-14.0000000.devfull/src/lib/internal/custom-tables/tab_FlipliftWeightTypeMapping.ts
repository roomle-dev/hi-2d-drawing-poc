...
export interface cti_tab_FlipliftWeightTypeMapping {
...
export interface ctm_tab_FlipliftWeightTypeMapping {
}

export interface cto_tab_FlipliftWeightTypeMapping extends ctm_tab_FlipliftWeightTypeMapping {
  readonly _id : number;
  WeightType(frontHeight:number, frontWeight:number, handleWeight:number):number;
  readonly CalculationType? : string;
}

export interface ICT_tab_FlipliftWeightTypeMapping
extends cti_tab_FlipliftWeightTypeMapping, cto_tab_FlipliftWeightTypeMapping {}

export class ct2_tab_FlipliftWeightTypeMapping {
...
public findExactly(
     ...
     in_HeightMin: number,
     in_HeightMax: number,
): cto_tab_FlipliftWeightTypeMapping | undefined {
  const res = ct_tab_FlipliftWeightTypeMapping.find((p) => 
...
public find(
predicate: (value: cti_tab_FlipliftWeightTypeMapping) => boolean
): cto_tab_FlipliftWeightTypeMapping | undefined {
  for (let index = 0; index < ct_tab_FlipliftWeightTypeMapping.length; index++){
    const element = ct_tab_FlipliftWeightTypeMapping[index];
...
export var ct_tab_FlipliftWeightTypeMapping :ICT_tab_FlipliftWeightTypeMapping[] = [
{
 _id: 12,
 in_FlipliftType: "FHF",
 ...
 in_HeightMax: 9999,
  WeightType(frontHeight:number, frontWeight:number, handleWeight:number):number {
    return frontHeight * (frontWeight + handleWeight);
  }
,
 CalculationType: "Set"
}
...
,{
 ...
 in_HeightMax: 9999,
  WeightType(frontHeight:number, frontWeight:number, handleWeight:number):number {
    return frontHeight * (frontWeight + handleWeight * 2);
  }
,

...
//#region Imports
import { cbp_mc_HangerSystem01, dc_mc_HangerSystem01, adc_mc_HangerSystem01 } from '../internal/modules/mc_HangerSystem01'
...
export function mc_HangerSystem01_createBuildPlan(this: cbp_mc_HangerSystem01):void {
  internal_enterModuleCreateBuildPlan('mc_HangerSystem01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mc_HangerSystem01_CREATEBUILDPLAN
// Schuler Consulting
// Schuler Consulting
// Create: Jun 2024
// By Joao Lisboa
...
// Get Hanger Data
//====================================================================
let hangerInfo = JSON.parse(this.mod_HangerInfo[0]);


//====================================================================
// Hanger Drawing + WallPlate Drawing + CoverCap Drawing
//====================================================================
let hangerGraphLibMap = GlobalFunc.find_GraphicLibraryMapping(hangerInfo.GraphicItem);
let hangerDrawing: ICT_tab_GraphicLibrary[] = [];
hangerGraphLibMap.forEach(x => {
  hangerDrawing.push(GlobalFunc.find_GraphicLibrary(x.Model3DGroupName!))
});

hangerDrawing.forEach(x => {
  if (this.mod_HangerPosX_matrix.Left && (x.Identifier == 'HangerLeftVisible' || x.Identifier == 'HangerLeftInvisible')) {
    let positionHangerLeft = GlobalFunc.process_GraphicInsertionHelper("020", x.in_Model3DGroupName!, x.DimensionX!, x.DimensionY!, x.DimensionZ!);
    ...
    if (x.Identifier == 'HangerLeftInvisible')
    {
      posz = - x.DimensionZ - this.mod_BackwallThk
    }
    ...
    hangerLeft.assign3DModel(x.Model3D!);
    ...
    hangerLeftDrill.pa_Model3DGroupName = x.in_Model3DGroupName!;
    hangerLeftDrill.pa_ProcessingId = hangerInfo.ProcessingItem!;
  }

  if (this.mod_HangerPosX_matrix.Right && (x.Identifier == 'HangerRightVisible' || x.Identifier == 'HangerRightInvisible')) {
    let positionHangerRight = GlobalFunc.process_GraphicInsertionHelper("220", x.in_Model3DGroupName!, x.DimensionX!, x.DimensionY!, x.DimensionZ!);
    ...
    if (x.Identifier == 'HangerRightInvisible')
    {
      posz = - x.DimensionZ - this.mod_BackwallThk
    }
    ...
    hangerRight.assign3DModel(x.Model3D!);
    ...
    hangerRightDrill.pa_Model3DGroupName = x.in_Model3DGroupName!;
    hangerRightDrill.pa_ProcessingId = hangerInfo.ProcessingItem!;
  }
  if (this.mod_HangerPosX_matrix.Left && x.Identifier == 'WallPlate' && hangerInfo.IncludeWallPlateLeft) {
    let tmpDimX = x.DimensionX!;
    // Cuttable WallPlate has a different dimx
    if (hangerInfo.WallPlateLeftLength != 0) {
      tmpDimX = hangerInfo.WallPlateLeftLength;
    }

    let positionWallPlateLeft = GlobalFunc.process_GraphicInsertionHelper("020", x.in_Model3DGroupName!, tmpDimX, x.DimensionY!, x.DimensionZ!);
    ...
    wallPlateLeft.assign3DModel(x.Model3D!);
  }
  if (this.mod_HangerPosX_matrix.Right && x.Identifier == 'WallPlate' && hangerInfo.IncludeWallPlateRight) {
    let tmpDimX = x.DimensionX!;
    ...
    let positionWallPlateRight = GlobalFunc.process_GraphicInsertionHelper("220", x.in_Model3DGroupName!, tmpDimX, x.DimensionY!, x.DimensionZ!);
    ...
    wallPlateRight.assign3DModel(x.Model3D!);
  }
  if (this.mod_HangerPosX_matrix.Left && x.Identifier == 'HangerCoverCapLeft') {
    let positionHangerCoverCapLeft = GlobalFunc.process_GraphicInsertionHelper("020", x.in_Model3DGroupName!, x.DimensionX!, x.DimensionY!, x.DimensionZ!);
  ...
  if (this.mod_HangerPosX_matrix.Right && x.Identifier == 'HangerCoverCapRight') {
    let positionHangerCoverCapRight = GlobalFunc.process_GraphicInsertionHelper("220", x.in_Model3DGroupName!, x.DimensionX!, x.DimensionY!, x.DimensionZ!);
...
// ---------------------------------------------------------------
export function mc_HangerSystem01_afterDataCompletion(this: adc_mc_HangerSystem01):void {
  internal_enterModuleAfterDataCompletion('mc_HangerSystem01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_HangerSystem01_AFTERDATACOMPLETION
...
// ---------------------------------------------------------------
export function mc_HangerSystem01_manufacturerDataCompletion(this: dc_mc_HangerSystem01):void {
  internal_enterModuleManufacturerDataCompletion('mc_HangerSystem01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_HangerSystem01_MANUFACTURERDATACOMPLETION

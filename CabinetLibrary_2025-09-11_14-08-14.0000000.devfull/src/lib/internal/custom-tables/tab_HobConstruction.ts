...
export interface cti_tab_HobConstruction {
  readonly in_ConstructionId? : string;
}

export interface ctm_tab_HobConstruction {
}

export interface cto_tab_HobConstruction extends ctm_tab_HobConstruction {
  readonly _id : number;
  readonly CutWidth : number;
  readonly CutDepth : number;
  readonly CutRadius : number;
  readonly HobBlockedHgt : number;
}

export interface ICT_tab_HobConstruction
extends cti_tab_HobConstruction, cto_tab_HobConstruction {}

export class ct2_tab_HobConstruction {

public findExactly(
     in_ConstructionId: string|undefined,
): cto_tab_HobConstruction | undefined {
  const res = ct_tab_HobConstruction.find((p) => 
...
public find(
predicate: (value: cti_tab_HobConstruction) => boolean
): cto_tab_HobConstruction | undefined {
  for (let index = 0; index < ct_tab_HobConstruction.length; index++){
    const element = ct_tab_HobConstruction[index];
...
export var ct_tab_HobConstruction :ICT_tab_HobConstruction[] = [
{
 _id: 1,
 in_ConstructionId: "PUXU",
 CutWidth: 810,
 CutDepth: 495,
 CutRadius: 5,
 HobBlockedHgt: 220

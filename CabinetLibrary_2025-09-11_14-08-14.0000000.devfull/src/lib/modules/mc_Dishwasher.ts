...
//#region Imports
import { cbp_mc_Dishwasher, dc_mc_Dishwasher, adc_mc_Dishwasher } from '../internal/modules/mc_Dishwasher'
...
export function mc_Dishwasher_createBuildPlan(this: cbp_mc_Dishwasher):void {
  internal_enterModuleCreateBuildPlan('mc_Dishwasher', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mc_Dishwasher_CREATEBUILDPLAN
...
// ---------------------------------------------------------------
export function mc_Dishwasher_afterDataCompletion(this: adc_mc_Dishwasher):void {
  internal_enterModuleAfterDataCompletion('mc_Dishwasher', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_Dishwasher_AFTERDATACOMPLETION
  ...
  let Door: any;
  let DoorDesign = this.mod_FrontDesign == 'Automatic' ? retDoorInfo.FrontDesign! : this.mod_FrontDesign;

  // Add the module
  if (DoorDesign == "FlatPanel") {
    Door = this.addOD_M_mc_FrontPanel01();
  }
  else if (DoorDesign == "WoodFrame") {
  ...
  if (dwInfo.GraphicId) {
    ...
    // Set origin
    Graphic.setOrigin(retDoorInfo.WidthPos(this) + retDoorInfo.Width(this) / 2, this.mod_Originpos[3], 0);
  }
...
// ---------------------------------------------------------------
export function mc_Dishwasher_manufacturerDataCompletion(this: dc_mc_Dishwasher):void {
  internal_enterModuleManufacturerDataCompletion('mc_Dishwasher', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_Dishwasher_MANUFACTURERDATACOMPLETION

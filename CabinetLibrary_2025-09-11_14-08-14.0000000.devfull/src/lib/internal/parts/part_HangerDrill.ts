...
export interface IP_part_HangerDrill_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_HangerSystem01;
  get pa_Model3DGroupName(): string;
  get pa_ProcessingId(): string;
}

export interface IP_part_HangerDrill_PartVarsWritable extends IP_part_HangerDrill_PartVarsReadOnly {
  set pa_Model3DGroupName(value: string);
  set pa_ProcessingId(value: string);
}

export class P_part_HangerDrill extends PartBase implements IP_part_HangerDrill_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_HangerSystem01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_HangerDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_HangerDrill (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_HangerDrill (id: ' + this._id + ')');
    }

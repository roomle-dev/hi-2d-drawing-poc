...
export interface cti_tab_DishwasherConstruction {
  readonly in_ConstructionId? : string;
}

export interface ctm_tab_DishwasherConstruction {
}

export interface cto_tab_DishwasherConstruction extends ctm_tab_DishwasherConstruction {
  readonly _id : number;
...
export interface ICT_tab_DishwasherConstruction
extends cti_tab_DishwasherConstruction, cto_tab_DishwasherConstruction {}

export class ct2_tab_DishwasherConstruction {

public findExactly(
     in_ConstructionId: string|undefined,
): cto_tab_DishwasherConstruction | undefined {
  const res = ct_tab_DishwasherConstruction.find((p) => 
...
public find(
predicate: (value: cti_tab_DishwasherConstruction) => boolean
): cto_tab_DishwasherConstruction | undefined {
  for (let index = 0; index < ct_tab_DishwasherConstruction.length; index++){
    const element = ct_tab_DishwasherConstruction[index];
...
export var ct_tab_DishwasherConstruction :ICT_tab_DishwasherConstruction[] = [
{
 _id: 9,
 in_ConstructionId: "SN55ZS07CE",
 NominalWidth: 600,
 MinHeight: 815,
 MaxHeight: 900,
 MinDepth: 550,

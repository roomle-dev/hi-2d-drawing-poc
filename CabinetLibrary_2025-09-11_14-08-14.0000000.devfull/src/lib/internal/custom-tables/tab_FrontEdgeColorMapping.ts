...
export interface cti_tab_FrontEdgeColorMapping {
  readonly in_FrontProgram? : string;
  readonly in_FrontColor? : string;
}

export interface ctm_tab_FrontEdgeColorMapping {
}

export interface cto_tab_FrontEdgeColorMapping extends ctm_tab_FrontEdgeColorMapping {
  readonly _id : number;
  readonly StandardColor? : string;
}

export interface ICT_tab_FrontEdgeColorMapping
extends cti_tab_FrontEdgeColorMapping, cto_tab_FrontEdgeColorMapping {}

export class ct2_tab_FrontEdgeColorMapping {

public findExactly(
     in_FrontProgram: string|undefined,
     in_FrontColor: string|undefined,
): cto_tab_FrontEdgeColorMapping | undefined {
  const res = ct_tab_FrontEdgeColorMapping.find((p) => 
...
public find(
predicate: (value: cti_tab_FrontEdgeColorMapping) => boolean
): cto_tab_FrontEdgeColorMapping | undefined {
  for (let index = 0; index < ct_tab_FrontEdgeColorMapping.length; index++){
    const element = ct_tab_FrontEdgeColorMapping[index];
...
export var ct_tab_FrontEdgeColorMapping :ICT_tab_FrontEdgeColorMapping[] = [
{
 _id: 1,
 in_FrontProgram: "All",
 in_FrontColor: "316",

...
//#region Imports
import { cbp_mc_ShelfadjGroup01, dc_mc_ShelfadjGroup01, adc_mc_ShelfadjGroup01 } from '../internal/modules/mc_ShelfadjGroup01'
import { GlobalFunc } from '../internal/global-func'
import { dc_mc_ShelfadjWood01, OD_M_mc_ShelfadjWood01 } from '../internal/modules/mc_ShelfadjWood01'
import { dc_mc_ShelfadjGlass01, OD_M_mc_ShelfadjGlass01 } from '../internal/modules/mc_ShelfadjGlass01'
import { dc_mc_ShelfadjDrill01, OD_M_mc_ShelfadjDrill01 } from '../internal/modules/mc_ShelfadjDrill01'
import { dc_mc_ShelfadjFitting01, OD_M_mc_ShelfadjFitting01 } from '../internal/modules/mc_ShelfadjFitting01'
...
export function mc_ShelfadjGroup01_createBuildPlan(this: cbp_mc_ShelfadjGroup01):void {
  internal_enterModuleCreateBuildPlan('mc_ShelfadjGroup01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mc_ShelfadjGroup01_CREATEBUILDPLAN
...
// ---------------------------------------------------------------
export function mc_ShelfadjGroup01_afterDataCompletion(this: adc_mc_ShelfadjGroup01):void {
  internal_enterModuleAfterDataCompletion('mc_ShelfadjGroup01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_ShelfadjGroup01_AFTERDATACOMPLETION
  ...
  // Revisions:
  // By Ludwig Weber
  // Revision of the complete script
  //===================================================
  ...
  let AdjShelfInfo = GlobalFunc.process_ShelfadjPos(this);
  let Panel: any;

  // Add the module for the adjustable shelves
  if (AdjShelfInfo.StartPosY.length  > 0){
    for(let i=1; i<=AdjShelfInfo.Qty; i++){

      // Insert the specific adjustable shelf based on the module type
      if (AdjShelfInfo.ModuleType == "Wood"){
        Panel = this.addOD_M_mc_ShelfadjWood01(); 
      }
      else if (AdjShelfInfo.ModuleType == "Glass"){
        Panel = this.addOD_M_mc_ShelfadjGlass01(); 
      }
      
      // Move the module into the correct positions
      Panel.setOrigin(AdjShelfInfo.StartPosX + AdjShelfInfo.DrillSettings.ShelfadjWidthReduction/2,AdjShelfInfo.StartPosY[i],AdjShelfInfo.StartPosZ + AdjShelfInfo.OffsetBack)

      // Set attributes of the child
      Panel.mod_Width = this.mod_Width - AdjShelfInfo.DrillSettings.ShelfadjWidthReduction;
      Panel.mod_Depth = this.mod_Depth - AdjShelfInfo.OffsetFront - AdjShelfInfo.OffsetBack;
      Panel.mod_ShelfadjThk = AdjShelfInfo.ShelfThk;
      Panel.mod_CarcaseId = this.mod_CarcaseId;
      Panel.mod_ShelfadjWoodColor = AdjShelfInfo.Color;     
    }
  }
  ...
  if (AdjShelfInfo.StartPosY.length  > 0){
    for(let i=1; i<=AdjShelfInfo.Qty; i++){
      ...
      // Move the module into the correct positions
      Fitting.setOrigin(0,AdjShelfInfo.StartPosY[i],AdjShelfInfo.StartPosZ)

      // Set attributes of the child
      Fitting.mod_Width = this.mod_Width - AdjShelfInfo.DrillSettings.ShelfadjWidthReduction;
      Fitting.mod_Depth = this.mod_Depth - 0 - 0;
      ...
      assignDrillingAttributes(Fitting, AdjShelfInfo.DrillSettings, this, AdjShelfInfo.DrillDistance);
    }
  }
  ...
  if (AdjShelfInfo.QtyDrills == 0) {
    let Drillings = this.addOD_M_mc_ShelfadjDrill01();
    Drillings.mod_Height = this.mod_Height - this.g.basic_ShelfadjDrillinglineOffsetBtm - this.g.basic_ShelfadjDrillinglineOffsetTop;
    Drillings.setOrigin(0, this.g.basic_ShelfadjDrillinglineOffsetBtm, 0);
    assignDrillingAttributes(Drillings, AdjShelfInfo.DrillSettings, this, AdjShelfInfo.DrillDistance);
  } 
  ...
  // Group of drills
    ...
    if (AdjShelfInfo.StartPosY.length > 0) {
      for (let i = 1; i <= AdjShelfInfo.Qty; i++) {
        ...
        let HalfQty = Math.floor((AdjShelfInfo.QtyDrills - 1) / 2)
        Drillings.setOrigin(0, AdjShelfInfo.StartPosY[i] - AdjShelfInfo.DrillDistance * HalfQty, 0);
        assignDrillingAttributes(Drillings, AdjShelfInfo.DrillSettings, this, AdjShelfInfo.DrillDistance);
      }
    }
  }
  ...
  function assignDrillingAttributes(Drillings: any, settings: any, m: any, DrillDistance: number) {
    ...
    Drillings.mod_Width = m.mod_Width;

    if (m.mod_VertDividerType == "Cleat" || m.mod_VertDividerType == "MiddleSideShort"){
      Drillings.mod_Depth = m.mod_Depth;
    }
    else{
      Drillings.mod_Depth = m.mod_Depth - AdjShelfInfo.OffsetFront;
    }
    ...
    //Drillings.mod_ShelfadjDrillDistance = settings.ShelfadjDrillDistance;
    Drillings.mod_ShelfadjDrillDistance = DrillDistance;
  }
...
// ---------------------------------------------------------------
export function mc_ShelfadjGroup01_manufacturerDataCompletion(this: dc_mc_ShelfadjGroup01):void {
  internal_enterModuleManufacturerDataCompletion('mc_ShelfadjGroup01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_ShelfadjGroup01_MANUFACTURERDATACOMPLETION

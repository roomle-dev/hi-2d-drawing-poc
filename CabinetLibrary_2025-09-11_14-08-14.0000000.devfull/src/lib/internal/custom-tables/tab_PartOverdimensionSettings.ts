...
export interface cti_tab_PartOverdimensionSettings {
  readonly in_EdgeThkMin : number;
  readonly in_EdgeThkMax : number;
}

export interface ctm_tab_PartOverdimensionSettings {
}

export interface cto_tab_PartOverdimensionSettings extends ctm_tab_PartOverdimensionSettings {
  readonly _id : number;
  readonly Overdimension : number;
}

export interface ICT_tab_PartOverdimensionSettings
extends cti_tab_PartOverdimensionSettings, cto_tab_PartOverdimensionSettings {}

export class ct2_tab_PartOverdimensionSettings {

public findExactly(
     in_EdgeThkMin: number,
     in_EdgeThkMax: number,
): cto_tab_PartOverdimensionSettings | undefined {
  const res = ct_tab_PartOverdimensionSettings.find((p) => 
p.in_EdgeThkMin === in_EdgeThkMin
 && p.in_EdgeThkMax === in_EdgeThkMax
);
return res;
}

public find(
predicate: (value: cti_tab_PartOverdimensionSettings) => boolean
): cto_tab_PartOverdimensionSettings | undefined {
  for (let index = 0; index < ct_tab_PartOverdimensionSettings.length; index++){
    const element = ct_tab_PartOverdimensionSettings[index];
...
export var ct_tab_PartOverdimensionSettings :ICT_tab_PartOverdimensionSettings[] = [
{
 _id: 7,
 in_EdgeThkMin: 0.8,
 in_EdgeThkMax: 0.8,

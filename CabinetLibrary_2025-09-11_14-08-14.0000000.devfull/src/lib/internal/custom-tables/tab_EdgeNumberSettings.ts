...
export interface cti_tab_EdgeNumberSettings {
  readonly in_PartName? : string;
  readonly in_Edge? : string;
}

export interface ctm_tab_EdgeNumberSettings {
}

export interface cto_tab_EdgeNumberSettings extends ctm_tab_EdgeNumberSettings {
  readonly _id : number;
  readonly BomEdgeDescription? : string;
  readonly BomEdgeNumber? : string;
  readonly BomEdgeType? : string;
}

export interface ICT_tab_EdgeNumberSettings
extends cti_tab_EdgeNumberSettings, cto_tab_EdgeNumberSettings {}

export class ct2_tab_EdgeNumberSettings {

public findExactly(
     in_PartName: string|undefined,
     in_Edge: string|undefined,
): cto_tab_EdgeNumberSettings | undefined {
  const res = ct_tab_EdgeNumberSettings.find((p) => 
p.in_PartName === in_PartName
 && p.in_Edge === in_Edge
);
return res;
}
...
public find(
predicate: (value: cti_tab_EdgeNumberSettings) => boolean
): cto_tab_EdgeNumberSettings | undefined {
  for (let index = 0; index < ct_tab_EdgeNumberSettings.length; index++){
    const element = ct_tab_EdgeNumberSettings[index];
...
export var ct_tab_EdgeNumberSettings :ICT_tab_EdgeNumberSettings[] = [
{
 _id: 1,
 in_PartName: "All",
 in_Edge: "EdgeFront",
 BomEdgeDescription: "Edge South",
 BomEdgeNumber: "1",
 BomEdgeType: "Edge1"
}
,{

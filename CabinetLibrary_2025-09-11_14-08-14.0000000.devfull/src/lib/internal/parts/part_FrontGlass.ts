...
export interface IP_part_FrontGlass_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_FrontPanelGlass01;
}

export interface IP_part_FrontGlass_PartVarsWritable extends IP_part_FrontGlass_PartVarsReadOnly {
}

export class P_part_FrontGlass extends PartBase implements IP_part_FrontGlass_PartVarsWritable {
...
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FrontGlass', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }

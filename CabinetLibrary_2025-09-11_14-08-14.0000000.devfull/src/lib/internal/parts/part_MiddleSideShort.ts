...
import { P_part_HardwareShelfadjDrill, IP_part_HardwareShelfadjDrill_PartVarsReadOnly } from './part_HardwareShelfadjDrill'
...
export interface IP_part_MiddleSideShort_PartVarsReadOnly extends IPartBase {
  parent: IModBaseProp;
}

export interface IP_part_MiddleSideShort_PartVarsWritable extends IP_part_MiddleSideShort_PartVarsReadOnly {
}

export class P_part_MiddleSideShort extends PartBase implements IP_part_MiddleSideShort_PartVarsWritable {
  constructor(parent: IModBaseProp, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_MiddleSideShort', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'YZX';
  }
  parent: IModBaseProp; // module which created this part
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 447: part_PushtoopenDrill FromFront
    if (touch == PartTouch.FromFront && partNext instanceof P_part_PushtoopenDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT447 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_MiddleSideShort_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
...
GlobalFunc.ops_PushtoopenDrill('Carcase', this, partSelf, part2, posRel);
      ...
      try {
      // ncout_Group: 451: part_PushtoopenMountingPlateDrill FromRight
    if (touch == PartTouch.FromRight && partNext instanceof P_part_PushtoopenMountingPlateDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT451 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_MiddleSideShort_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenMountingPlateDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
GlobalFunc.ops_PushtoopenDrill('Carcase', this, partSelf, part2, posRel);
      ...
      try {
      // ncout_Group: 459: part_HardwareShelfadjDrill FromLeft
    if (touch == PartTouch.FromLeft && partNext instanceof P_part_HardwareShelfadjDrill) {
        let part2 = <P_part_HardwareShelfadjDrill>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT459 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_MiddleSideShort_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HardwareShelfadjDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);

...
import { IPartParentsNonNull_mc_Handle01 } from '../mod-interfaces'

export interface IP_part_HandleDrill_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Handle01;
  get pa_Model3DGroupName(): string;
  get pa_ProcessingId(): string;
  get pa_Rotation(): number;
  get pa_Length(): number;
}

export interface IP_part_HandleDrill_PartVarsWritable extends IP_part_HandleDrill_PartVarsReadOnly {
  set pa_Model3DGroupName(value: string);
  set pa_ProcessingId(value: string);
  set pa_Rotation(value: number);
  set pa_Length(value: number);
}

export class P_part_HandleDrill extends PartBase implements IP_part_HandleDrill_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Handle01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_HandleDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Handle01; // module which created this part
  ...
  res.set('pa_Length', this.#pa_Length);
    return res;
  }
  setAttributes() {
    try {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_HandleDrill_ValueScript
    if (this.#pa_Model3DGroupName === undefined) {
      this.pa_Model3DGroupName = this.parent.mod_Model3DGroupName;
    }
    if (this.#pa_ProcessingId === undefined) {
      this.pa_ProcessingId = this.parent.mod_ProcessingId;
    }
    if (this.#pa_Rotation === undefined) {
      this.pa_Rotation = this.parent.mod_Rotation;
    }
    if (this.#pa_Length === undefined) {
      this.pa_Length = 0;
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
  ...
  checkAttributes() {
    ...
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_HandleDrill (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_HandleDrill (id: ' + this._id + ')');
    }
    if (this.#pa_Rotation === undefined) {
      logError('Attribute pa_Rotation not set in part part_HandleDrill (id: ' + this._id + ')');
    }
    if (this.#pa_Length === undefined) {
      logError('Attribute pa_Length not set in part part_HandleDrill (id: ' + this._id + ')');
    }
  ...
  #pa_Rotation?: number;
  get pa_Length(): number { return this.#pa_Length!; }
  set pa_Length(val:number) { this.#pa_Length = val; }
  #pa_Length?: number;
}

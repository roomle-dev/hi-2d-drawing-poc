...
export interface IP_part_ToekickGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Toekick;
  get pa_BomId(): string;
}

export interface IP_part_ToekickGroup_PartVarsWritable extends IP_part_ToekickGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_ToekickGroup extends PartBase implements IP_part_ToekickGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Toekick, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_ToekickGroup', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_ToekickGroup (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 188
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM188 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_ToekickGroup_PartVarsReadOnly) {
            const part = partSelf;

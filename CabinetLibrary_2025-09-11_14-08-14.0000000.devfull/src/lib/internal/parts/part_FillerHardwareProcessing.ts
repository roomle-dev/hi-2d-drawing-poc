...
import { IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01 } from '../mod-interfaces'

export interface IP_part_FillerHardwareProcessing_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01;
  get pa_ProcessingId(): string;
  get pa_FrontGapCarcase(): number;
  get pa_Model3DGroupName(): string;
  get pa_Rotation(): number;
  get pa_HardwareIdentifier(): string;
}

export interface IP_part_FillerHardwareProcessing_PartVarsWritable extends IP_part_FillerHardwareProcessing_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
  set pa_FrontGapCarcase(value: number);
  set pa_Model3DGroupName(value: string);
  set pa_Rotation(value: number);
  set pa_HardwareIdentifier(value: string);
}

export class P_part_FillerHardwareProcessing extends PartBase implements IP_part_FillerHardwareProcessing_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FillerHardwareProcessing', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01; // module which created this part
  ...
  res.set('pa_Rotation', this.#pa_Rotation);
  res.set('pa_HardwareIdentifier', this.#pa_HardwareIdentifier);
    return res;
  }
  setAttributes() {
    try {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FillerHardwareProcessing_ValueScript
    ...
    if (this.#pa_Rotation === undefined) {
      this.pa_Rotation = 0;
    }
    if (this.#pa_HardwareIdentifier === undefined) {
      this.pa_HardwareIdentifier = '';
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_FillerHardwareProcessing (id: ' + this._id + ')');
    }
    if (this.#pa_FrontGapCarcase === undefined) {
      logError('Attribute pa_FrontGapCarcase not set in part part_FillerHardwareProcessing (id: ' + this._id + ')');
    }
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_FillerHardwareProcessing (id: ' + this._id + ')');
    }
    if (this.#pa_Rotation === undefined) {
      logError('Attribute pa_Rotation not set in part part_FillerHardwareProcessing (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareIdentifier === undefined) {
      logError('Attribute pa_HardwareIdentifier not set in part part_FillerHardwareProcessing (id: ' + this._id + ')');
    }
  ...
  get pa_Rotation(): number { return this.#pa_Rotation!; }
  set pa_Rotation(val:number) { this.#pa_Rotation = val; }
  #pa_Rotation?: number;
  get pa_HardwareIdentifier(): string { return this.#pa_HardwareIdentifier!; }
  set pa_HardwareIdentifier(val:string) { this.#pa_HardwareIdentifier = val; }
  #pa_HardwareIdentifier?: string;
}

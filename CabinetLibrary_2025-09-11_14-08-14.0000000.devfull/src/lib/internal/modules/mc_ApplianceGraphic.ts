...
import { IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood } from '../mod-interfaces'
import { IP_part_ApplianceGraphic_PartVarsWritable, P_part_ApplianceGraphic } from '../parts/part_ApplianceGraphic'
import { IModuleNonNull_mc_ApplianceGraphic,IModParents_mc_ApplianceGraphic,IPartParentsNonNull_mc_ApplianceGraphic} from '../mod-interfaces'
import {IModVar_mod_GraphicId,IModVarNonNull_mod_GraphicId} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_GraphicId} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_ApplianceGraphic } from '../part-add-interfaces';
import { mc_ApplianceGraphic_createBuildPlan, mc_ApplianceGraphic_afterDataCompletion, mc_ApplianceGraphic_manufacturerDataCompletion } from '../../modules/mc_ApplianceGraphic';

export interface cbp_mc_ApplianceGraphic extends IModBaseProp
, IPartAdd_part_ApplianceGraphic, IModVarNonNull_mod_GraphicId {
  parent: IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood;
...
export interface dc_mc_ApplianceGraphic extends IModBaseProp
, IModVar_mod_GraphicId {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mc_ApplianceGraphic;
  parent: IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood;
}

export interface adc_mc_ApplianceGraphic extends IModBaseProp
, IModVarNonNull_mod_GraphicId {
  get m(): OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
}

export class OD_M_mc_ApplianceGraphic extends OD_Base  implements dc_mc_ApplianceGraphic
,IModParents_mc_ApplianceGraphic
,IModVar_mod_GraphicId
 {
  constructor(parent: IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood, manufacturerMode?:boolean) {
    super('mc_ApplianceGraphic', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_ApplianceGraphic_NonNull(this);
  }
  parent: IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood;
  _selfNonNull: OD_M_mc_ApplianceGraphic_NonNull;

  override variants:string[] = ['mod_GraphicId',];
  #mod_GraphicId?: string;
  get mod_GraphicId(): string | undefined { return this.#mod_GraphicId}
  set mod_GraphicId(value: string | undefined) {
    if(this._adcFinished){
      logError("The property 'mod_GraphicId' must not be changed after the module was completed with 'afterDataCompletion'");
      return;
    }
    if(this.#mod_GraphicId === value) return;
    this.#mod_GraphicId = value;
  }

  mod_GraphicId_matrix?: IMatrix_mod_GraphicId;
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('mod_GraphicId', this.mod_GraphicId);
    return res;
  }
  override loadJson(json:any, withSubModules:boolean = true, manufacturerMode:boolean) {
    ...
    { internal_enterValidateVariant(this.modId, this._id, 'mod_GraphicId');
    const val = _toString(json['attributes']['mod_GraphicId'], 'mod_GraphicId');
    internal_leaveValidateVariant();
    this.mod_GraphicId = val; }
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_GraphicId === undefined) {
      this.mod_GraphicId = "None";
    }
  ...
  #manufacturerDataCompletionInternal = mc_ApplianceGraphic_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_ApplianceGraphic_NonNull(this);
  ...
  seal(): IModuleNonNull_mc_ApplianceGraphic{
    ...
    const adc = new OD_M_mc_ApplianceGraphic_NonNull(this);
    return adc;
  }

  override validateSelections():void {
    this.mod_GraphicId_matrix = VariantValidation.mod_GraphicId(this, this);
  }
  addpart_ApplianceGraphic(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_ApplianceGraphic_PartVarsWritable {
    var c = new P_part_ApplianceGraphic(this._selfNonNull, x, y, z, dimx, dimy, dimz);
    this.p.push(c);
    return c;
  }

  override createBuildPlan():void {
    var cbp = new OD_M_mc_ApplianceGraphic_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_ApplianceGraphic_NonNull implements cbp_mc_ApplianceGraphic, adc_mc_ApplianceGraphic ,IPartParentsNonNull_mc_ApplianceGraphic {
  constructor(parent: OD_M_mc_ApplianceGraphic) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_ApplianceGraphic;
  ...
  #createBuildPlanInternal = mc_ApplianceGraphic_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_ApplianceGraphic_afterDataCompletion;
  ...
  parent: IModParents_mc_Dishwasher_mc_Fridge01_mc_Hob01_mc_Oven01_mc_Sink01_mf_Sink_mr_Hood;
  addpart_ApplianceGraphic(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_ApplianceGraphic_PartVarsWritable {
    return this.#internalParent.addpart_ApplianceGraphic(x, y, z, dimx, dimy, dimz);
  }
  get mod_GraphicId(): string  {
    return this.#internalParent.mod_GraphicId!;
  }
}

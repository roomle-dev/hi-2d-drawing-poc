...
export interface IP_part_DrawerBoxDrilling_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_DrawerBox01;
  get pa_ProcessingId(): string;
  get pa_ProcessingId2(): string;
}

export interface IP_part_DrawerBoxDrilling_PartVarsWritable extends IP_part_DrawerBoxDrilling_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
  set pa_ProcessingId2(value: string);
}

export class P_part_DrawerBoxDrilling extends PartBase implements IP_part_DrawerBoxDrilling_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerBoxDrilling', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_DrawerBoxDrilling_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_DrawerBoxDrilling (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId2 === undefined) {
      logError('Attribute pa_ProcessingId2 not set in part part_DrawerBoxDrilling (id: ' + this._id + ')');
    }

...
import { IP_part_LightSystem_PartVarsWritable, P_part_LightSystem } from '../parts/part_LightSystem'
import { IModuleNonNull_mc_LightSystem01,IModParents_mc_LightSystem01,IPartParentsNonNull_mc_LightSystem01} from '../mod-interfaces'
import {IModVar_mod_Depth,IModVarNonNull_mod_Depth,IModVar_mod_Height,IModVarNonNull_mod_Height,IModVar_mod_Width,IModVarNonNull_mod_Width,IModVar_mod_LightSystem,IModVarNonNull_mod_LightSystem,IModVar_mod_ModuleName,IModVarNonNull_mod_ModuleName} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_Depth, IMatrix_mod_Height, IMatrix_mod_Width, IMatrix_mod_LightSystem, IMatrix_mod_ModuleName} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_LightSystem } from '../part-add-interfaces';
import { mc_LightSystem01_createBuildPlan, mc_LightSystem01_afterDataCompletion, mc_LightSystem01_manufacturerDataCompletion } from '../../modules/mc_LightSystem01';

export interface cbp_mc_LightSystem01 extends IModBaseProp
, IPartAdd_part_LightSystem, IModVarNonNull_mod_Depth, IModVarNonNull_mod_Height, IModVarNonNull_mod_Width, IModVarNonNull_mod_LightSystem, IModVarNonNull_mod_ModuleName {
...
export interface dc_mc_LightSystem01 extends IModBaseProp
, IModVar_mod_Depth, IModVar_mod_Height, IModVar_mod_Width, IModVar_mod_LightSystem, IModVar_mod_ModuleName {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mc_LightSystem01;
  parent: IModParents_mc_Storageunit01;
}

export interface adc_mc_LightSystem01 extends IModBaseProp
...
export class OD_M_mc_LightSystem01 extends OD_Base  implements dc_mc_LightSystem01
,IModParents_mc_LightSystem01
,IModVar_mod_Depth,IModVar_mod_Height,IModVar_mod_Width,IModVar_mod_LightSystem,IModVar_mod_ModuleName
 {
  constructor(parent: IModParents_mc_Storageunit01, manufacturerMode?:boolean) {
    super('mc_LightSystem01', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_LightSystem01_NonNull(this);
  }
  parent: IModParents_mc_Storageunit01;
  _selfNonNull: OD_M_mc_LightSystem01_NonNull;

  override variants:string[] = ['mod_Depth','mod_Height','mod_Width','mod_LightSystem','mod_ModuleName',];
  #mod_Depth?: number;
  ...
  #dataCompletionAssignDerivedData() {
    ...
    if (this.mod_LightSystem === undefined) {
      ...
      while (s !== undefined) {
        if (s.variants.includes('mod_LightSystem')) {
          let pv = <IModVar_mod_LightSystem>s;
          if (pv.mod_LightSystem !== undefined) {
            this.mod_LightSystem = (<IModVar_mod_LightSystem>s).mod_LightSystem;
            break;
          };
        }
        s = s.parentBase;
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_ModuleName === undefined) {
      this.mod_ModuleName = "mc_LightSystem01";
    }
  ...
  #manufacturerDataCompletionInternal = mc_LightSystem01_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_LightSystem01_NonNull(this);
  ...
  seal(): IModuleNonNull_mc_LightSystem01{
    ...
    const adc = new OD_M_mc_LightSystem01_NonNull(this);
    return adc;
  }
  ...
  addpart_LightSystem(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_LightSystem_PartVarsWritable {
    var c = new P_part_LightSystem(this._selfNonNull, x, y, z, dimx, dimy, dimz);
    this.p.push(c);
    return c;
  }

  override createBuildPlan():void {
    var cbp = new OD_M_mc_LightSystem01_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_LightSystem01_NonNull implements cbp_mc_LightSystem01, adc_mc_LightSystem01 ,IPartParentsNonNull_mc_LightSystem01 {
  constructor(parent: OD_M_mc_LightSystem01) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_LightSystem01;
  ...
  #createBuildPlanInternal = mc_LightSystem01_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_LightSystem01_afterDataCompletion;
  ...
  parent: IModParents_mc_Storageunit01;
  addpart_LightSystem(x:number, y:number, z:number, dimx:number, dimy:number, dimz:number): IP_part_LightSystem_PartVarsWritable {
    return this.#internalParent.addpart_LightSystem(x, y, z, dimx, dimy, dimz);
  }

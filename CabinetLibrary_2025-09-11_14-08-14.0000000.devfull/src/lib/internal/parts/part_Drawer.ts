...
export interface IP_part_Drawer_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_Drawer_PartVarsWritable extends IP_part_Drawer_PartVarsReadOnly {
...
export class P_part_Drawer extends PartBase implements IP_part_Drawer_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Drawer', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Drawer (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareInfo === undefined) {
      logError('Attribute pa_HardwareInfo not set in part part_Drawer (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM173 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Drawer_PartVarsReadOnly) {
            const part = partSelf;
...
GlobalFunc.process_BoardBom(this, part, elementType, elementCategory, parentBomId);
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 344: part_HandleDrill FromFront
    if (touch == PartTouch.FromFront && partNext instanceof P_part_HandleDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT344 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Drawer_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HandleDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
...
GlobalFunc.ncout_HandleDrill(this, partSelf, part2, posRel)
      ...
      try {
      // ncout_Group: 393: part_DrawerBoxDrilling FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_DrawerBoxDrilling) {
        let part2 = <P_part_DrawerBoxDrilling>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT393 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Drawer_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerBoxDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
drillings.forEach(x => 
  {  
    ...
    drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU/2 + '" /></svg>', 'z');
      ...
      try {
      // ncout_Group: 394: part_DrawerShelfbtmConnectorDrilling FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_DrawerShelfbtmConnectorDrilling) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT394 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Drawer_PartVarsReadOnly, touch: PartTouch, part2: IP_part_DrawerShelfbtmConnectorDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);

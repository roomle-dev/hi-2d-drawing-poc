...
export interface cti_tab_FramePartConnectionMapping {
  readonly in_PartGroup? : string;
  readonly in_TypeElement? : string;
  readonly in_FrontProgram? : string;
  readonly in_ThkMin : number;
  readonly in_ThkMax : number;
}

export interface ctm_tab_FramePartConnectionMapping {
}

export interface cto_tab_FramePartConnectionMapping extends ctm_tab_FramePartConnectionMapping {
  readonly _id : number;
  readonly ConnectionHardwarePositioning? : string;
  readonly Object? : string;
}

export interface ICT_tab_FramePartConnectionMapping
extends cti_tab_FramePartConnectionMapping, cto_tab_FramePartConnectionMapping {}

export class ct2_tab_FramePartConnectionMapping {
...
public findExactly(
     ...
     in_ThkMin: number,
     in_ThkMax: number,
): cto_tab_FramePartConnectionMapping | undefined {
  const res = ct_tab_FramePartConnectionMapping.find((p) => 
p.in_PartGroup === in_PartGroup
 && p.in_TypeElement === in_TypeElement
 && p.in_FrontProgram === in_FrontProgram
 && p.in_ThkMin === in_ThkMin
 && p.in_ThkMax === in_ThkMax
);
return res;
}

public find(
predicate: (value: cti_tab_FramePartConnectionMapping) => boolean
): cto_tab_FramePartConnectionMapping | undefined {
  for (let index = 0; index < ct_tab_FramePartConnectionMapping.length; index++){
    const element = ct_tab_FramePartConnectionMapping[index];
...
export var ct_tab_FramePartConnectionMapping :ICT_tab_FramePartConnectionMapping[] = [
{
 _id: 3,
 in_PartGroup: "All",
 in_TypeElement: "All",
 ...
 in_ThkMin: 16,
 in_ThkMax: 99,
 ConnectionHardwarePositioning: "FramePartConnection01",
 Object: "Dowel02"
}
,{
 _id: 4,
 ...
 in_ThkMin: 16,
 in_ThkMax: 99,
 ConnectionHardwarePositioning: "FramePartConnection02",
 Object: "Dowel02"
}
];

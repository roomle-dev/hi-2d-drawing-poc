...
import { IPartParentsNonNull_mc_ShelfadjFitting01_mc_ShelfadjGroup01 } from '../mod-interfaces'

export interface IP_part_ShelfadjFitting_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_ShelfadjFitting01_mc_ShelfadjGroup01;
  get pa_HardwareId(): string;
}

export interface IP_part_ShelfadjFitting_PartVarsWritable extends IP_part_ShelfadjFitting_PartVarsReadOnly {
  set pa_HardwareId(value: string);
}

export class P_part_ShelfadjFitting extends PartBase implements IP_part_ShelfadjFitting_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_ShelfadjFitting01_mc_ShelfadjGroup01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_ShelfadjFitting', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_ShelfadjFitting01_mc_ShelfadjGroup01; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_ShelfadjFitting_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_ShelfadjFitting (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM156 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_ShelfadjFitting_PartVarsReadOnly) {
            const part = partSelf;
            internal_enterBomPartMasterDataElements(partSelf._id, partSelf._partId);
             // ####################### CUSTOM SCRIPTS ########################
// Filter function to get all the hardware elements for the adjustable shelf
...
HardwareElements.forEach(info => {
  ...
  let ShelfadjFittingBOMInfo = GlobalFunc.find_HardwareLibrary(info.SupplierArticleNumber!,info.Supplier!);
  let ShelfadjFittingBOMName = GlobalFunc.find_PartSettings("part_ShelfadjFitting");

  elem.bom_Supplier=info.Supplier;
  elem.bom_SupplierArticle=info.SupplierArticleNumber;
  elem.bom_Description1=ShelfadjFittingBOMInfo.Description;
  elem.bom_Description2=ShelfadjFittingBOMInfo.Description2;
  elem.bom_Length=ShelfadjFittingBOMInfo.Length;
  elem.bom_Width=ShelfadjFittingBOMInfo.Width;
  elem.bom_Thk=ShelfadjFittingBOMInfo.Thickness;
  elem.bom_Color=ShelfadjFittingBOMInfo.Color;
  elem.bom_Weight=ShelfadjFittingBOMInfo.Weight;
  elem.bom_Name=ShelfadjFittingBOMName.BomPartDescription;
  ...
  elem.bom_Qty=info.BomQty;
        ...
        _bom._description = "Add all the elements for the adjustable shelf to BOM";
        _bom.set(partSelf);

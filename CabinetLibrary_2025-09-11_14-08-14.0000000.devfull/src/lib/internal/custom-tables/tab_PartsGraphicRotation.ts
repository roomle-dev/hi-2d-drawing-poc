...
export interface cti_tab_PartsGraphicRotation {
  readonly in_ReferencePointXYZ? : string;
  readonly in_RotationAxis? : string;
  readonly in_Rotation : number;
}

export interface ctm_tab_PartsGraphicRotation {
}

export interface cto_tab_PartsGraphicRotation extends ctm_tab_PartsGraphicRotation {
  readonly _id : number;
  AdditionalX(DimX:number, DimY:number, DimZ:number):number;
  AdditionalY(DimX:number, DimY:number, DimZ:number):number;
  AdditionalZ(DimX:number, DimY:number, DimZ:number):number;
  RotationVectorX(DimX:number, DimY:number, DimZ:number):number;
  RotationVectorY(DimX:number, DimY:number, DimZ:number):number;
  RotationVectorZ(DimX:number, DimY:number, DimZ:number):number;
}

export interface ICT_tab_PartsGraphicRotation
extends cti_tab_PartsGraphicRotation, cto_tab_PartsGraphicRotation {}

export class ct2_tab_PartsGraphicRotation {

public findExactly(
     in_ReferencePointXYZ: string|undefined,
     in_RotationAxis: string|undefined,
     in_Rotation: number,
): cto_tab_PartsGraphicRotation | undefined {
  const res = ct_tab_PartsGraphicRotation.find((p) => 
p.in_ReferencePointXYZ === in_ReferencePointXYZ
 && p.in_RotationAxis === in_RotationAxis
 && p.in_Rotation === in_Rotation
);
return res;
}

public find(
predicate: (value: cti_tab_PartsGraphicRotation) => boolean
): cto_tab_PartsGraphicRotation | undefined {
  for (let index = 0; index < ct_tab_PartsGraphicRotation.length; index++){
    const element = ct_tab_PartsGraphicRotation[index];
...
export var ct_tab_PartsGraphicRotation :ICT_tab_PartsGraphicRotation[] = [
{
 _id: 13,
 in_ReferencePointXYZ: "012",
 in_RotationAxis: "Z",
 in_Rotation: 0,
  AdditionalX(DimX:number, DimY:number, DimZ:number):number {
    return 0;

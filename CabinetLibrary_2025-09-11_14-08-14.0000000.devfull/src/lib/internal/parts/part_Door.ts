...
export interface IP_part_Door_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_Door_PartVarsWritable extends IP_part_Door_PartVarsReadOnly {
...
export class P_part_Door extends PartBase implements IP_part_Door_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Door', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_DoorDirection === undefined) {
      logError('Attribute pa_DoorDirection not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Door (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareInfo === undefined) {
      logError('Attribute pa_HardwareInfo not set in part part_Door (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM200 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Door_PartVarsReadOnly) {
            const part = partSelf;
...
// Type of the Bom-Element (will be checked in order output)
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 515: part_HandleDrill FromFront
    if (touch == PartTouch.FromFront && partNext instanceof P_part_HandleDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT515 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Door_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HandleDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
...
GlobalFunc.ncout_HandleDrill(this, partSelf, part2, posRel)
      ...
      try {
      // ncout_Group: 516: part_HingeDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_HingeDrill) {
        let part2 = <P_part_HingeDrill>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT516 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Door_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HingeDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
//====================================================================
  ...
  {  
    ...
    if (posRel.x < partSelf._dimx / 2) {
      ...
      try {
      // ncout_Group: 517: part_PushtoopenDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_PushtoopenDrill) {
        let part2 = <P_part_PushtoopenDrill>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT517 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Door_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
GlobalFunc.ops_PushtoopenDrill('Front', this, partSelf, part2, posRel);
      ...
      try {
      // ncout_Group: 518: part_PushtoopenMountingPlateDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_PushtoopenMountingPlateDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT518 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Door_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenMountingPlateDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);

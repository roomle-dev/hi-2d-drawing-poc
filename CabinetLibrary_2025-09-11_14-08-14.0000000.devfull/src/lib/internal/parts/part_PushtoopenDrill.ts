...
export interface IP_part_PushtoopenDrill_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_PushtoopenDrill_PartVarsWritable extends IP_part_PushtoopenDrill_PartVarsReadOnly {
  set pa_Model3DGroupName(value: string);
  set pa_ProcessingId(value: string);
  set pa_Rotation(value: number);
}

export class P_part_PushtoopenDrill extends PartBase implements IP_part_PushtoopenDrill_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Pushtoopen01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_PushtoopenDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_PushtoopenDrill_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_PushtoopenDrill (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_PushtoopenDrill (id: ' + this._id + ')');
    }
    if (this.#pa_Rotation === undefined) {
      logError('Attribute pa_Rotation not set in part part_PushtoopenDrill (id: ' + this._id + ')');
    }

...
export interface IP_part_HingeDoor_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Hinge01;
  get pa_HardwareId(): string;
  get pa_TopColor(): string;
}

export interface IP_part_HingeDoor_PartVarsWritable extends IP_part_HingeDoor_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_TopColor(value: string);
}

export class P_part_HingeDoor extends PartBase implements IP_part_HingeDoor_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Hinge01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_HingeDoor', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_HingeDoor (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_HingeDoor (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM158 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_HingeDoor_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  let BOMName = GlobalFunc.find_PartSettings("part_HingeDoor");

...
export interface cti_tab_BracketMapping {
  readonly in_Supplier? : string;
  readonly in_BracketId? : string;
}

export interface ctm_tab_BracketMapping {
}

export interface cto_tab_BracketMapping extends ctm_tab_BracketMapping {
  readonly _id : number;
  readonly ConstructionId? : string;
  readonly GraphicId? : string;
}

export interface ICT_tab_BracketMapping
extends cti_tab_BracketMapping, cto_tab_BracketMapping {}

export class ct2_tab_BracketMapping {

public findExactly(
     in_Supplier: string|undefined,
     in_BracketId: string|undefined,
): cto_tab_BracketMapping | undefined {
  const res = ct_tab_BracketMapping.find((p) => 
p.in_Supplier === in_Supplier
 && p.in_BracketId === in_BracketId
);
return res;
}

public find(
predicate: (value: cti_tab_BracketMapping) => boolean
): cto_tab_BracketMapping | undefined {
  for (let index = 0; index < ct_tab_BracketMapping.length; index++){
    const element = ct_tab_BracketMapping[index];
...
export var ct_tab_BracketMapping :ICT_tab_BracketMapping[] = [
{
 _id: 4,
 in_Supplier: "Homag",
 in_BracketId: "Floating",
 ConstructionId: "brFloating",
 GraphicId: "Model3DbrFloating"
}
,{
 _id: 5,
 in_Supplier: "Homag",
 in_BracketId: "ARRAS5",
 ConstructionId: "brARRAS5",
 GraphicId: "Model3DbrARRAS5"
}
,{
 _id: 6,
 in_Supplier: "Homag",
 in_BracketId: "Tucano",
 ConstructionId: "brTucano",
 GraphicId: "Model3DbrTucano"
}
];

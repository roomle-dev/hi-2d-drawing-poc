...
export interface IP_part_CountertopGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Countertop01;
  get pa_BomId(): string;
}

export interface IP_part_CountertopGroup_PartVarsWritable extends IP_part_CountertopGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_CountertopGroup extends PartBase implements IP_part_CountertopGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Countertop01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_CountertopGroup', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XZY';
    this._hidden = true;
  }
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_CountertopGroup_ValueScript
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
    } 
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_CountertopGroup (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 185
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM185 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_CountertopGroup_PartVarsReadOnly) {
            const part = partSelf;

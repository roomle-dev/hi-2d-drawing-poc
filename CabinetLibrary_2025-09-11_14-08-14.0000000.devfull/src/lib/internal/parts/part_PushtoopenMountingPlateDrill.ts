...
import { IPartParentsNonNull_mc_Pushtoopen01 } from '../mod-interfaces'

export interface IP_part_PushtoopenMountingPlateDrill_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Pushtoopen01;
  get pa_Model3DGroupName(): string;
  get pa_ProcessingId(): string;
  get pa_Rotation(): number;
}

export interface IP_part_PushtoopenMountingPlateDrill_PartVarsWritable extends IP_part_PushtoopenMountingPlateDrill_PartVarsReadOnly {
  set pa_Model3DGroupName(value: string);
  set pa_ProcessingId(value: string);
  set pa_Rotation(value: number);
}

export class P_part_PushtoopenMountingPlateDrill extends PartBase implements IP_part_PushtoopenMountingPlateDrill_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Pushtoopen01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_PushtoopenMountingPlateDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Pushtoopen01; // module which created this part
  ...
  setAttributes() {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_PushtoopenMountingPlateDrill_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_Model3DGroupName === undefined) {
      logError('Attribute pa_Model3DGroupName not set in part part_PushtoopenMountingPlateDrill (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_PushtoopenMountingPlateDrill (id: ' + this._id + ')');
    }
    if (this.#pa_Rotation === undefined) {
      logError('Attribute pa_Rotation not set in part part_PushtoopenMountingPlateDrill (id: ' + this._id + ')');
    }

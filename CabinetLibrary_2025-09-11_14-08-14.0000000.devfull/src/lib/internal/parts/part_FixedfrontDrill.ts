...
export interface IP_part_FixedfrontDrill_PartVarsReadOnly extends IPartBase {
  parent: IModBaseProp;
}

export interface IP_part_FixedfrontDrill_PartVarsWritable extends IP_part_FixedfrontDrill_PartVarsReadOnly {
}

export class P_part_FixedfrontDrill extends PartBase implements IP_part_FixedfrontDrill_PartVarsWritable {
  constructor(parent: IModBaseProp, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FixedfrontDrill', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }

...
export interface IP_part_Filler_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_Filler_PartVarsWritable extends IP_part_Filler_PartVarsReadOnly {
...
export class P_part_Filler extends PartBase implements IP_part_Filler_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Filler', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Filler (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareInfo === undefined) {
      logError('Attribute pa_HardwareInfo not set in part part_Filler (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM215 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Filler_PartVarsReadOnly) {
            const part = partSelf;
...
// Type of the Bom-Element (will be checked in order output)
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 629: part_FillerHardwareProcessing FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_FillerHardwareProcessing) {
        let part2 = <P_part_FillerHardwareProcessing>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT629 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Filler_PartVarsReadOnly, touch: PartTouch, part2: IP_part_FillerHardwareProcessing_PartVarsReadOnly, posRel:CollisionDataRel) {
...
//====================================================================
  ...
  override createBomTouchElementsEnd() : void { 
      try {
        const partSelf = this;
        if (partSelf.parent.mod_FrontType == 'CornerFillerLeft') { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT635 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Filler_PartVarsReadOnly) {
              internal_enterBomPartMasterDataTouchesEnd(partSelf._id, partSelf._partId);
               // ####################### CUSTOM SCRIPTS ########################
//====================================================================
  // Get fillerHardwareInfo
  //====================================================================
  ...
  let fillerHardwareInfo: iFillerLeftHardwareInfo = JSON.parse(partSelf.pa_HardwareInfo);

  let i: number = 0;
  fillerHardwareInfo.ProcessingItem.forEach(fillerHardware => {
    //====================================================================
    // Get data for drillings and rotation
    //====================================================================
    ...
    let drillings: any[] = [];
    let drills: any[] = [];
    let processings = GlobalFunc.find_ProcessingMapping(fillerHardware);
    processings.forEach((processing) => {
      drills = GlobalFunc.find_HardwareDrillVertLibrary(processing.ProcessingId!, 'Front');
      drills.forEach((drill) => {
        drillings.push(drill);
    ...
    drillings.forEach(x => {
      ...
      let DrillVert = this.addncout_DrillVert();
      DrillVert.nc_TOOL = "102";
      DrillVert.nc_XA = fillerHardwareInfo.PosX[i] + x.XA;
      DrillVert.nc_YA = fillerHardwareInfo.PosY[i] + x.YA;
      DrillVert.nc_TI = x.TI;
      DrillVert.nc_DU = x.DU;
      DrillVert.nc_BM = x.matrix_BM || "LS"
      DrillVert.nc_WI = x.matrix_WI ?? 0;
      ...
      // Add drawing
      let drilling01 = partSelf.add3DElement('Drilling01', DrillVert, fillerHardwareInfo.PosX[i] + x.XA, fillerHardwareInfo.PosY[i] + x.YA, 0, x.DU, x.DU, x.TI);
      drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU / 2 + '" /></svg>', 'z');
    ...
    i++
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Drillings for Corner filler left hardware";
          _bom.set(partSelf);
          partSelf._bom.push(_bom);
          if(_bom._subBom){
            _bom._subBom.forEach(p => partSelf._bom.push(p));

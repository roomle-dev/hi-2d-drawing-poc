...
import { OD_M_mc_Fingergrip01, dc_mc_Fingergrip01 } from './mc_Fingergrip01'
import { IModuleNonNull_mr_Fingergrip,IModParents_mr_Fingergrip,IPartParentsNonNull_mr_Fingergrip} from '../mod-interfaces'
import {IModVar_mod_FingergripColor,IModVarNonNull_mod_FingergripColor,IModVar_mod_FingergripType,IModVarNonNull_mod_FingergripType,IModVar_mod_LengthList,IModVarNonNull_mod_LengthList} from '../var-interfaces'
import { VariantValidation, IMatrix_mod_FingergripColor, IMatrix_mod_FingergripType} from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { mr_Fingergrip_createBuildPlan, mr_Fingergrip_afterDataCompletion, mr_Fingergrip_manufacturerDataCompletion } from '../../modules/mr_Fingergrip';

export interface cbp_mr_Fingergrip extends IModBaseProp
...
export interface dc_mr_Fingergrip extends IModBaseProp
, IModVar_mod_FingergripColor, IModVar_mod_FingergripType, IModVar_mod_LengthList {
  m:OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  seal():IModuleNonNull_mr_Fingergrip;
  getGenerationContours(): GenerationContour[];
  addOD_M_mc_Fingergrip01(index?: number): dc_mc_Fingergrip01;
}

export interface adc_mr_Fingergrip extends IModBaseProp
, IModVarNonNull_mod_FingergripColor, IModVarNonNull_mod_FingergripType, IModVarNonNull_mod_LengthList {
  get m(): OD_Base[];
  setOrigin(x:number|Matrix4,y?:number,z?:number):void;
  addOD_M_mc_Fingergrip01(index?: number): dc_mc_Fingergrip01;
...
export class OD_M_mr_Fingergrip extends OD_Base  implements dc_mr_Fingergrip
,IModParents_mr_Fingergrip
,IModVar_mod_FingergripColor,IModVar_mod_FingergripType,IModVar_mod_LengthList
 {
  constructor(parent?: OD_Base, manufacturerMode?:boolean) {
    super('mr_Fingergrip', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this._g = new GlobalVars();
    this._openGroups = new Map<string, OpenGroup>();
    this._partGroups = new Map<string, PartGroup>();
    this._selfNonNull = new OD_M_mr_Fingergrip_NonNull(this);
    this._isGenerated = true;
  }
  _selfNonNull: OD_M_mr_Fingergrip_NonNull;
  getGenerationContours(): GenerationContour[] { return this.generationContours; }

  override variants:string[] = ['mod_FingergripColor','mod_FingergripType','mod_LengthList',];
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_FingergripColor === undefined) {
      this.mod_FingergripColor = "240";
    }
    if (this.mod_FingergripType === undefined) {
      this.mod_FingergripType = "Wood";
    }
  ...
  #manufacturerDataCompletionInternal = mr_Fingergrip_manufacturerDataCompletion;
  override afterDataCompletion():void {
    if(this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mr_Fingergrip_NonNull(this);
  ...
  seal(): IModuleNonNull_mr_Fingergrip{
    ...
    const adc = new OD_M_mr_Fingergrip_NonNull(this);
    return adc;
  }

  addOD_M_mc_Fingergrip01(index?: number): dc_mc_Fingergrip01 {
    var m = new OD_M_mc_Fingergrip01(this);
    m.dataCompletion();
  ...
  override createBuildPlan():void {
    var cbp = new OD_M_mr_Fingergrip_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mr_Fingergrip_NonNull implements cbp_mr_Fingergrip, adc_mr_Fingergrip ,IPartParentsNonNull_mr_Fingergrip {
  constructor(parent: OD_M_mr_Fingergrip) {
    this.#internalParent = parent;
  }
  #internalParent: OD_M_mr_Fingergrip;
  ...
  #createBuildPlanInternal = mr_Fingergrip_createBuildPlan;
  afterDataCompletion():void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mr_Fingergrip_afterDataCompletion;
  ...
  addOD_M_mc_Fingergrip01(index?: number): dc_mc_Fingergrip01 {
    return this.#internalParent.addOD_M_mc_Fingergrip01(index);
  }
}

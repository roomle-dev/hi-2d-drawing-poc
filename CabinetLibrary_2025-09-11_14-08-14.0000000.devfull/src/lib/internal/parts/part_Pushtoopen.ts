...
export interface IP_part_Pushtoopen_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Pushtoopen01;
}

export interface IP_part_Pushtoopen_PartVarsWritable extends IP_part_Pushtoopen_PartVarsReadOnly {
}

export class P_part_Pushtoopen extends PartBase implements IP_part_Pushtoopen_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Pushtoopen01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Pushtoopen', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }

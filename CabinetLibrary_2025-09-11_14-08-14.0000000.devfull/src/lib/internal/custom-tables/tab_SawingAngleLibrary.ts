...
export interface cti_tab_SawingAngleLibrary {
  readonly in_ProcessingId? : string;
}

export interface ctm_tab_SawingAngleLibrary {
  readonly matrix_KO? : string;
  readonly matrix_NB? : number;
  readonly matrix_T_? : string;
}

export interface cto_tab_SawingAngleLibrary extends ctm_tab_SawingAngleLibrary {
  readonly _id : number;
  XA(varPos:number):number;
  YA(varPos:number):number;
  XE(varPos:number):number;
  YE(varPos:number):number;
  readonly XY : number;
  WI(varAngle:number):number;
  readonly EM? : string;
  readonly Z_ : number;
  readonly VZ : number;
  readonly RK? : string;
...
export interface ICT_tab_SawingAngleLibrary
extends cti_tab_SawingAngleLibrary, cto_tab_SawingAngleLibrary {}

export class ct2_tab_SawingAngleLibrary {

public findExactly(
     in_ProcessingId: string|undefined,
): cto_tab_SawingAngleLibrary | undefined {
  const res = ct_tab_SawingAngleLibrary.find((p) => 
...
public find(
predicate: (value: cti_tab_SawingAngleLibrary) => boolean
): cto_tab_SawingAngleLibrary | undefined {
  for (let index = 0; index < ct_tab_SawingAngleLibrary.length; index++){
    const element = ct_tab_SawingAngleLibrary[index];
...
export var ct_tab_SawingAngleLibrary :ICT_tab_SawingAngleLibrary[] = [
{
 _id: 3,
 in_ProcessingId: "AngledCutInLengthNegativeAngle",
  XA(varPos:number):number {
    return varPos;
  }
,
...
,{
 _id: 4,
 in_ProcessingId: "AngledCutInLengthPositiveAngle",
  XA(varPos:number):number {
    return varPos;
  }
,

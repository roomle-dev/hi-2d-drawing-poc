...
//#region Imports
import { cbp_mc_ShelfadjFitting01, dc_mc_ShelfadjFitting01, adc_mc_ShelfadjFitting01 } from '../internal/modules/mc_ShelfadjFitting01'
...
export function mc_ShelfadjFitting01_createBuildPlan(this: cbp_mc_ShelfadjFitting01):void {
  internal_enterModuleCreateBuildPlan('mc_ShelfadjFitting01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
// CUSTOMSCRIPT_mc_ShelfadjFitting01_CREATEBUILDPLAN
  ...
  let ObjectSideFront = this.mod_ShelfadjSidepanelProcessingFrontId;
  let ObjectSideMiddle = this.mod_ShelfadjSidepanelProcessingMiddleId;
  let ObjectSideBack = this.mod_ShelfadjSidepanelProcessingBackId;
  let ObjectBackwall = this.mod_ShelfadjBackwallProcessingId;
  let ObjectVertDivider = this.mod_ShelfadjVertDividerProcessingId;

  // Drilling lines on the sidepanels
  let DrillingLinesSide = GlobalFunc.process_Descriptor(this.mod_ShelfadjSidepanelDrillDescriptor, this.mod_Depth);

  // Drilling lines on the backwall
  let DrillingLinesBackwall = GlobalFunc.process_Descriptor(this.mod_ShelfadjBackwallDrillDescriptor, this.mod_Width);
  ...
  DrillingLinesSide.forEach(DrillingLine => {
    let ObjectInfo = k == 0 ? ObjectSideBack : k == DrillingLinesSide.length-1 ? ObjectSideFront : ObjectSideMiddle;
  ...
  function InsertPart(m: any, Side: string, Object: any, position: number){
    ...
    let HardwareObject = GlobalFunc.find_ObjectMapping(Object);
    let GraphicItems = GlobalFunc.find_GraphicLibraryMapping(HardwareObject.GraphicItem!)

    GraphicItems.forEach(GraphicItem => {
      GraphicInfo = GlobalFunc.find_GraphicLibrary(GraphicItem.Model3DGroupName!)
    })
    ...
    if (GraphicInfo !== undefined){
      PosX = GraphicInfo.PartOffsetX;
      PosY = -GraphicInfo.DimensionY;
      PosZ = position - GraphicInfo.DimensionZ/2
      DimX = GraphicInfo.DimensionX - GraphicInfo.PartOffsetX;
      DimY = GraphicInfo.DimensionY + GraphicInfo.PartOffsetY;
      DimZ = GraphicInfo.DimensionZ;
      PartOffsetX = GraphicInfo.PartOffsetX;
    }
    ...
    // Right sidepanel
      ...
      let partMatrix = PartHelper.rotateY(Elem!, 180, new Vector3(-PartOffsetX, 0, DimZ/2));
    ...
    // Backwall
      ...
      let partMatrix = PartHelper.rotateY(Elem!, 270, new Vector3(0, 0, 0));
      Elem!.setMatrix(partMatrix);
    }
    ...
    // Vertical divider
      ...
      let partMatrix = PartHelper.rotateY(Elem!, 90, new Vector3(0, 0, 0));
      Elem!.setMatrix(partMatrix);
    }

    // Add obj-File and assign to PartGroup
    if (Elem !== undefined) {
      if (GraphicInfo?.Model3D) {
          Elem.assign3DModel(GraphicInfo.Model3D);
      }
      m.assignPartGroup(m.mod_CarcaseId, Elem);
      Elem.pa_HardwareId = HardwareObject?.HardwareItem ?? '';

      // Add the material in the graphic
      let hwColor = GlobalFunc.process_HardwareColor(HardwareObject?.HardwareItem ?? '');
      if (hwColor) {
        GlobalFunc.process_AddMaterial(Elem, 'fittingShelfadj', hwColor);
      }
    }
  }
...
// ---------------------------------------------------------------
export function mc_ShelfadjFitting01_afterDataCompletion(this: adc_mc_ShelfadjFitting01):void {
  internal_enterModuleAfterDataCompletion('mc_ShelfadjFitting01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_ShelfadjFitting01_AFTERDATACOMPLETION
...
// ---------------------------------------------------------------
export function mc_ShelfadjFitting01_manufacturerDataCompletion(this: dc_mc_ShelfadjFitting01):void {
  internal_enterModuleManufacturerDataCompletion('mc_ShelfadjFitting01', this._id);
  try {
// ###############################################################
// ####################### CUSTOM SCRIPTS ########################
// ###############################################################
 // CUSTOMSCRIPT_mc_ShelfadjFitting01_MANUFACTURERDATACOMPLETION

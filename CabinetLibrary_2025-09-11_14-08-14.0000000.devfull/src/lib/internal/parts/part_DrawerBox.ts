...
export interface IP_part_DrawerBox_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_DrawerBox01;
}

export interface IP_part_DrawerBox_PartVarsWritable extends IP_part_DrawerBox_PartVarsReadOnly {
}

export class P_part_DrawerBox extends PartBase implements IP_part_DrawerBox_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_DrawerBox', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
  }

...
import { IPartParentsNonNull_mc_HangerSystem01 } from '../mod-interfaces'

export interface IP_part_HangerSystemBom_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_HangerSystem01;
  get pa_HardwareId(): string;
  get pa_WallPlateLengthLeft(): number;
  get pa_WallPlateLengthRight(): number;
  get pa_WallPlateQtyLeft(): number;
  get pa_WallPlateQtyRight(): number;
}

export interface IP_part_HangerSystemBom_PartVarsWritable extends IP_part_HangerSystemBom_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_WallPlateLengthLeft(value: number);
  set pa_WallPlateLengthRight(value: number);
  set pa_WallPlateQtyLeft(value: number);
  set pa_WallPlateQtyRight(value: number);
}

export class P_part_HangerSystemBom extends PartBase implements IP_part_HangerSystemBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_HangerSystem01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_HangerSystemBom', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_HangerSystem01; // module which created this part
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('pa_HardwareId', this.#pa_HardwareId);
  res.set('pa_WallPlateLengthLeft', this.#pa_WallPlateLengthLeft);
  res.set('pa_WallPlateLengthRight', this.#pa_WallPlateLengthRight);
  res.set('pa_WallPlateQtyLeft', this.#pa_WallPlateQtyLeft);
  res.set('pa_WallPlateQtyRight', this.#pa_WallPlateQtyRight);
    return res;
  }
  setAttributes() {
    try {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_HangerSystemBom_ValueScript
    if (this.#pa_HardwareId === undefined) {
      this.pa_HardwareId = '';
    }
    if (this.#pa_WallPlateLengthLeft === undefined) {
      this.pa_WallPlateLengthLeft = 0;
    }
    if (this.#pa_WallPlateLengthRight === undefined) {
      this.pa_WallPlateLengthRight = 0;
    }
    if (this.#pa_WallPlateQtyLeft === undefined) {
      this.pa_WallPlateQtyLeft = 0;
    }
    if (this.#pa_WallPlateQtyRight === undefined) {
      this.pa_WallPlateQtyRight = 0;
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_HangerSystemBom (id: ' + this._id + ')');
    }
    if (this.#pa_WallPlateLengthLeft === undefined) {
      logError('Attribute pa_WallPlateLengthLeft not set in part part_HangerSystemBom (id: ' + this._id + ')');
    }
    if (this.#pa_WallPlateLengthRight === undefined) {
      logError('Attribute pa_WallPlateLengthRight not set in part part_HangerSystemBom (id: ' + this._id + ')');
    }
    if (this.#pa_WallPlateQtyLeft === undefined) {
      logError('Attribute pa_WallPlateQtyLeft not set in part part_HangerSystemBom (id: ' + this._id + ')');
    }
    if (this.#pa_WallPlateQtyRight === undefined) {
      logError('Attribute pa_WallPlateQtyRight not set in part part_HangerSystemBom (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM176 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_HangerSystemBom_PartVarsReadOnly) {
            const part = partSelf;
...
HardwareElements.forEach(info => {
  ...
  get pa_HardwareId(): string { return this.#pa_HardwareId!; }
  set pa_HardwareId(val:string) { this.#pa_HardwareId = val; }
  #pa_HardwareId?: string;
  get pa_WallPlateLengthLeft(): number { return this.#pa_WallPlateLengthLeft!; }
  set pa_WallPlateLengthLeft(val:number) { this.#pa_WallPlateLengthLeft = val; }
  #pa_WallPlateLengthLeft?: number;
  get pa_WallPlateLengthRight(): number { return this.#pa_WallPlateLengthRight!; }
  set pa_WallPlateLengthRight(val:number) { this.#pa_WallPlateLengthRight = val; }
  #pa_WallPlateLengthRight?: number;
  get pa_WallPlateQtyLeft(): number { return this.#pa_WallPlateQtyLeft!; }
  set pa_WallPlateQtyLeft(val:number) { this.#pa_WallPlateQtyLeft = val; }
  #pa_WallPlateQtyLeft?: number;
  get pa_WallPlateQtyRight(): number { return this.#pa_WallPlateQtyRight!; }
  set pa_WallPlateQtyRight(val:number) { this.#pa_WallPlateQtyRight = val; }
  #pa_WallPlateQtyRight?: number;
}

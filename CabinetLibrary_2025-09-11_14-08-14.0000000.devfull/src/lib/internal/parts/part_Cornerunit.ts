...
import { IPartParentsNonNull_mc_Cornerunit01 } from '../mod-interfaces'

export interface IP_part_Cornerunit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Cornerunit01;
  get pa_BomId(): string;
}

export interface IP_part_Cornerunit_PartVarsWritable extends IP_part_Cornerunit_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_Cornerunit extends PartBase implements IP_part_Cornerunit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Cornerunit01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Cornerunit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Cornerunit01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Cornerunit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 222
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM222 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Cornerunit_PartVarsReadOnly) {
            const part = partSelf;
...
// Type of the Bom-Element (will be checked in order output)
  ...
  this.bom_ElementCategory=elementCategory
        ...
        _bom._description = "Part group for carcase";
        _bom.set(partSelf);

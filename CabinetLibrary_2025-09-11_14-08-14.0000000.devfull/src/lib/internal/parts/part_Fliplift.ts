...
import { IPartParentsNonNull_mc_FrontPanel01 } from '../mod-interfaces'

export interface IP_part_Fliplift_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_FrontPanel01;
  ...
  get pa_HardwareInfo(): string;
  get pa_FrontPanelConstruction(): cto_tab_FrontPanelConstruction|undefined;
}

export interface IP_part_Fliplift_PartVarsWritable extends IP_part_Fliplift_PartVarsReadOnly {
  ...
  set pa_HardwareInfo(value: string);
  set pa_FrontPanelConstruction(value: cto_tab_FrontPanelConstruction|undefined);
}

export class P_part_Fliplift extends PartBase implements IP_part_Fliplift_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_Fliplift', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  parent: IPartParentsNonNull_mc_FrontPanel01; // module which created this part
  ...
  res.set('pa_HardwareInfo', this.#pa_HardwareInfo);
  res.set('pa_FrontPanelConstruction', this.#pa_FrontPanelConstruction);
    return res;
  }
  setAttributes() {
    try {
    ...
    if (this.#pa_HardwareInfo === undefined) {
      this.pa_HardwareInfo = '';
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_FlipliftType === undefined) {
      logError('Attribute pa_FlipliftType not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_Fliplift (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareInfo === undefined) {
      logError('Attribute pa_HardwareInfo not set in part part_Fliplift (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    ...
    const partSelf = this;
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM193 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Fliplift_PartVarsReadOnly) {
            const part = partSelf;
...
GlobalFunc.process_BoardBom(this, part, elementType, elementCategory, parentBomId);
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
      try {
      // ncout_Group: 431: part_HandleDrill FromFront
    if (touch == PartTouch.FromFront && partNext instanceof P_part_HandleDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT431 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HandleDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
...
GlobalFunc.ncout_HandleDrill(this, partSelf, part2, posRel)
          ...
          _bom._description = "Drillings for the handle";
      ...
      try {
      // ncout_Group: 443: part_PushtoopenDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_PushtoopenDrill) {
        let part2 = <P_part_PushtoopenDrill>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT443 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
GlobalFunc.ops_PushtoopenDrill('Front', this, partSelf, part2, posRel);
      ...
      try {
      // ncout_Group: 444: part_PushtoopenMountingPlateDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_PushtoopenMountingPlateDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT444 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PushtoopenMountingPlateDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
GlobalFunc.ops_PushtoopenDrill('Front', this, partSelf, part2, posRel);
      ...
      try {
      // ncout_Group: 495: part_HingeDrill FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_HingeDrill) {
        let part2 = <P_part_HingeDrill>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT495 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HingeDrill_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
//====================================================================
  ...
  //====================================================================
    ...
    drillings.forEach(x => {
      ...
      try {
      // ncout_Group: 530: part_FlipliftHardwareFrontpanelDrilling FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_FlipliftHardwareFrontpanelDrilling) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT530 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_FlipliftHardwareFrontpanelDrilling_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
//====================================================================
  ...
  drillings.forEach(x => {
    ...
    drilling01.extrude('<svg><circle cx="' + 0 + '" cy="' + 0 + '" r="' + x.DU / 2 + '" /></svg>', 'z');
      ...
      try {
      // ncout_Group: 532: part_HingeDrillFHF FromBack
    if (touch == PartTouch.FromBack && partNext instanceof P_part_HingeDrillFHF) {
        let part2 = <P_part_HingeDrillFHF>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT532 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Fliplift_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HingeDrillFHF_PartVarsReadOnly, posRel:CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
...
if (posRel.y > 0) { // If the posRel.y is bigger than 0 then it's the bottom door and we insert the 35 mm drill (front)
  ...
  get pa_HardwareInfo(): string { return this.#pa_HardwareInfo!; }
  set pa_HardwareInfo(val:string) { this.#pa_HardwareInfo = val; }
  #pa_HardwareInfo?: string;
  get pa_FrontPanelConstruction(): cto_tab_FrontPanelConstruction|undefined { return this.#pa_FrontPanelConstruction; }
  set pa_FrontPanelConstruction(val:cto_tab_FrontPanelConstruction|undefined) { this.#pa_FrontPanelConstruction = val; }
  #pa_FrontPanelConstruction?: cto_tab_FrontPanelConstruction;
}

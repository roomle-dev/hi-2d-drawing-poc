...
import { BOM_ElementType_bomout_PartGroup } from '../bom/types/bom_bomout_PartGroup'
...
import { IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01_mf_CornerFillerFront } from '../mod-interfaces'

export interface IP_part_FillerUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01_mf_CornerFillerFront;
  get pa_BomId(): string;
  get pa_PartgroupBomId(): string;
}

export interface IP_part_FillerUnit_PartVarsWritable extends IP_part_FillerUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
  set pa_PartgroupBomId(value: string);
}

export class P_part_FillerUnit extends PartBase implements IP_part_FillerUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01_mf_CornerFillerFront, 
x:number, y:number, z:number, dimx:number, dimy:number, dimz:number) {
    super(parent, 'part_FillerUnit', x, y, z, dimx, dimy, dimz);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_CornerFillerFront01_mc_FillerFront01_mf_CornerFillerFront; // module which created this part
  override getAttributes() : Map<string,any> { 
    let res = new Map<string,any>();
  res.set('pa_BomId', this.#pa_BomId);
  res.set('pa_PartgroupBomId', this.#pa_PartgroupBomId);
    return res;
  }
  setAttributes() {
    try {
    ...
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // part_FillerUnit_ValueScript
    if (this.#pa_BomId === undefined) {
      this.pa_BomId = this.parent.mod_FrontId;
    }
    if (this.#pa_PartgroupBomId === undefined) {
      this.pa_PartgroupBomId = this.parent.mod_CarcaseId;
    }
    // ###############################################################
    // ################### END CUSTOM SCRIPTS ########################
    // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_FillerUnit (id: ' + this._id + ')');
    }
    if (this.#pa_PartgroupBomId === undefined) {
      logError('Attribute pa_PartgroupBomId not set in part part_FillerUnit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
    const partSelf = this;
      // bomout_PartGroup: 217
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM217 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_FillerUnit_PartVarsReadOnly) {
            const part = partSelf;
...
let elementType='Partgroup';

// ProductionSite Category (to split the orders for different production sites)
let elementCategory='';

// Id of the parent to create the hierarchy
let parentId1=part.pa_PartgroupBomId;

// Id of this element to identify the childs of this element
let elementId=part.pa_BomId;

// Name of the PartGroup
let name='Partgroup FillerFront'

this.bom_ElementId=elementId
this.bom_ElementType=elementType;
this.bom_ParentId=parentId1;
this.bom_Length=part._width;
this.bom_Width=part._depth;
this.bom_Thk = part._thickness;
this.bom_Name=name;
this.bom_PartId=part._id;
this.bom_ElementCategory=elementCategory
             // ################### END CUSTOM SCRIPTS ########################
            internal_leaveBomPartMasterDataElements();
          }
        }
        _bom._description = "PartGroup for Filler Front";
        _bom.set(partSelf);
  ...
  #pa_BomId?: string;
  get pa_PartgroupBomId(): string { return this.#pa_PartgroupBomId!; }
  set pa_PartgroupBomId(val:string) { this.#pa_PartgroupBomId = val; }
  #pa_PartgroupBomId?: string;
}
